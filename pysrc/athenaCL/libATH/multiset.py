...
def forteToSc(card, index, inversion=-2):
    ...
    if card > 12 or card < 1:
        ...
        scCard = card  # all other cards are good + used
...
class Multiset:
    ...
    def _update(self):
        ...
        if normData == None:  # an erro has happend
            environment.printDebug(["_update: problem w/", self._access("psReal")])
            raise error.MultisetError
        self._scTriple, self.tRef = normData
    ...
    def rotate(self, newZero):
        ...
        if newZero > 0:  # map as positive mod
            self._psList = self._psList[newZero:psLen] + self._psList[0:newZero]
        if newZero < 0:  # map as negative mod
            self._psList = (
                self._psList[psLen + newZero : psLen]
...
class MultisetFactory:
    ...
    def _parseSetInputType(self, usrStr, termObj):
        ...
        if usrStr in ["file", "import", "spectrum"]:
            if not termObj.interact:
                return None  # cant import w/ not interactive
            return "import"
    ...
    def _parseMidi(self, usrStr):
        ...
        for elem in usrList:  # may be int or float
            ...
            if elem == None:
                ...
                psList.append(pitchTools.midiToPs(elem))
        return psList
    ...
    def _parseFq(self, usrStr):
        ...
        for elem in usrList:  # may be int or float
            ...
            if elem == None:
                ...
                psList.append(pitchTools.fqToPs(elem))
        return psList
    ...
    def _parseSieve(self, usrStr):
        try:
            ...
            psSet = sieveObj()
        except (SyntaxError, ValueError, TypeError, KeyError, error.PitchSyntaxError):
            raise error.MultisetError
    ...
    def _getCount(self, termObj):
        ...
        while 1:
            ...
            if num != None and num != 0:
                ...
                dialog.msgOut(
                    ("%senter a positive or negative integer.\n" % lang.TAB), termObj
                )
        ...
        while 1:
            ...
            try:
                ...
                # import will get a file dialog
                    ...
                    if ok:
                        psSet = self._parseTxt(msg, count)
                    else:
                        return None  # cancel
                else:
                    return None
