...
class Texture(object):
    ...
    def stateUpdate(self, tCurrent, chordCurrent, ps, multisetObj, pitchObj, pitchPost):
    ...
    def updateTemperament(self, name):  # assing particular tuning
        ...
        if name == None:
            raise ValueError("bad temperament name error")
    ...
    def load(
        ...
        if midiPgm == None:
            self.midiPgm = 0  # default, added 1.0.23
        else:
            self.midiPgm = midiPgm
        if midiCh == None:
            self.midiCh = None  # default, added 1.0.23
        else:
            self.midiCh = midiCh
        self.mute = mute
        ...
        for auxLabel in basePmtr.auxLabel(self.auxNo):
            self.pmtrObjDict[auxLabel] = parameter.factory(self.pmtrQDict[auxLabel])
        ...
        for textLabel in self.textLabels:  # load object into textPmtr object
            ...
            self.pmtrObjDict[textLabel] = parameter.factory(
                args, "textPmtrObjs", self.refDict
            )
        ...
        for i, dynLabel in basePmtr.dynLabel(self.dynPmtrNo, 1):
            ...
            self.pmtrObjDict[dynLabel] = parameter.factory(args, type, self.refDict)
    ...
    def reprDoc(self, format=""):
        ...
        if format in [""]:  # format used by TIdoc command
            ...
            for textLabel in self.textLabels:
                msg.append(
                    (
                        self.pmtrObjDict[textLabel].type,
    ...
    def _reprDocInst(self):
        ...
        entryLines = []
        if "inst" not in self.pmtrActive:
            return entryLines  # empty if no inst?
    ...
    def _reprDoc(self):
        ...
        auxInterleave = drawer.listInterleave(
            self._reprDocAuxNames(), self._reprDocAuxArgs(), 1
        )
    ...
    def _reprView(self, extData):
        ...
        headList.append(
            "TI: %s, TM: %s, TC: %i, TT: %s\n"
            % (self.name, self.tmName, extData["cloneNo"], self.temperamentName)
        )
        headList.append("pitchMode: %s, " % (self.getPitchMode()))  # no return
        headList.append(
            "silenceMode: %s, postMapMode: %s\n"
            % (typeset.boolAsStr(self.silenceMode), typeset.boolAsStr(self.orcMapMode))
        )
    ...
    def clockFindPos(self, t):
        ...
        for i in range(len(self.clockPath)):
            ...
            return None  # error, t not in any time range of this texture
    ...
    def updateInstPmtr(self, inst=None, orcName="csoundNative", auxNo=None):
        ...
        environment.printDebug(
            [lang.WARN, "new Texture auxiliary value %s" % self.auxNo]
        )
    ...
    def updatePmtrObj(self, pmtrName=""):
        ...
        if pmtrName in [
            "tRange",
        ]:  # path should be here but its not a pmtr obj
            self._clockUpdate()
        elif pmtrName == "path":
            self.updatePathInfo()
        return ok, msg
    ...
    def _editRestore(self, attrName, pmtrName, data):
        ...
        if not ok:
            raise ValueError("texture edit: original data cannot be restored: %s" % msg)
    ...
    def editPmtrObj(self, pmtrName, pmtrValue, refresh=1):
        ...
        if p == "path":  # path is an object reference, not a string
            ...
            p
            in ("ampQ", "panQ", "fieldQ", "octQ", "beatT", "tRange", "inst", "rhythmQ")
            or p[:4] in ["auxQ", "dynQ"]
            or p[:5] == "textQ"
        ):
            if p[:4] == "auxQ":
        ...
        try:  # try to refresh objects
            ...
            error.ParameterObjectSyntaxError
        ) as msg:  # standard init errors from pmtr obj
    ...
    def makeEvent(
        self, tCurrent, bpm, pulse, dur, sus, acc, amp, ps, pan, auxiliary, comment=None
    ):
