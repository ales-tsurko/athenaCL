...
class HarmonicAssembly(baseTexture.Texture):
    def __init__(self, name=None):
        ...
        self.textPmtrNames = [
            "maxTimeOffset",
            "levelFieldPolyphonic",
            "levelOctavePolyphonic",
        ]
    ...
    def _scoreMain(self):
        ...
        while tCurrent < tEnd:
            ...
            pathPos = (
                abs(
                    drawer.floatToInt(
                        self.getTextDynamic("multisetPosition", tCurrent), "weight"
                    )
                )
                % pLen
            )
            ...
            for k in range(multisetCount):
                ...
                self.stateUpdate(
                    tCurrent, chordCurrent, None, multisetCurrent, None, None
                )
                ...
                for i in range(simultaneityCount):  # pitch in chord
                    ...
                    chordPos = abs(
                        drawer.floatToInt(
                            self.getTextDynamic("pitchPosition", tCurrent), "weight"
                        )
                    ) % len(chordCurrent)
