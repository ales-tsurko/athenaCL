...
class TimeSegment(baseTexture.Texture):
    def __init__(self, name=None):
        ...
        self.dynPmtrManifest = [
            {
                "name": "segmentWidthGenerator",
                "type": "genPmtrObjs",
                "default": ("bg", "rc", (1, 2, 3)),
                "doc": "segment width weight",
            },
            {
                "name": "eventCountGenerator",
                "type": "genPmtrObjs",
                "default": ("cg", "ud", 3, 40, 2),
    ...
    def _scoreMain(self):
        ...
        if textLevelEventCount == "segment":  # event count per segement
            ...
            if segEventCount <= 0:
                segEventCount = 1  # force minimum per seg
            for q in range(textTotalSegmentCount):
        ...
        for segPos in range(textTotalSegmentCount):
            ...
            for i in range(segEventCount):  #
                ...
                selectorChordPos = basePmtr.Selector(
                    list(range(0, len(chordCurrent))), textPitchSelectorControl
                )
