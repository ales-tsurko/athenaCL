...
def encodeMp3(
    ...
    tagStr = '--tt "%s" --ta "%s" --tl "%s" --ty "%s" ' % (title, artist, album, year)
    cmd = "lame -m s -q %s --add-id3v2 --cbr --disptime 1 -b %s -c %s %s %s" % (
        quality,
        br,
        tagStr,
        src,
...
def encodeOgg(src, dstDir=None, br=128, title="", artist="", album="", year=""):
    """encode using oggenc
...
def encodeAac(
    src, dstDir=None, br=128, title="", artist="", album="", year="", dstName=None
):
    ...
    tagStr = '--title "%s" --artist "%s" --album "%s" --year "%s" ' % (
        title,
        artist,
        album,
        year,
...
def soxFade(src, dst=None, timeIn=0.01, timeOut=0.01):
...
def soxAmp(src, dst=None, amp=0.9):
    # this is destructive if dst id none
    if dst == None:  # replace original
        nameStub, ext = osTools.extSplit(src)
        dst = "%s-temp%s" % (nameStub, ext)
...
def soxSpeed(src, dst=None, speed=0.5):
    # this is destructive if dst id none
    if dst == None:  # replace original
        nameStub, ext = osTools.extSplit(src)
        dst = "%s-temp%s" % (nameStub, ext)
...
class AudioFile:
    ...
    def _open(self, mode="r", ch=None):
        ...
        if mode == "r":
            ...
            if ch != None:
                self.ch = ch  # coerce channel if provided
    ...
    def insertNoise(self, pos, frames, amp=1):
        ...
        if frames + pos > self.frames:
            raise ValueError("bad position size")
    ...
    def testAmp(self, pos, frames, rmsThresh=0.08, maxThresh=0.50):
        ...
        if rms >= (absMax * rmsThresh) or max >= (absMax * maxThresh):
            ...
            print(
                _MOD,
                "lowamp: rms %s max %s (rmsPct %.3f)"
                % (str(rms).ljust(5), str(max).ljust(5), rmsPct),
            )
            result = 0
        return result
    ...
    def findShard(self, length, maxAttempt=20):
        ...
        if rangeMax <= 0:
            print(_MOD, "findShard: self.frames, length %s %s" % (self.frames, length))
            return None  # skip
...
class EncodeChain:
    ...
    def run(self):
        ...
        for i in range(self.steps):
            ...
            tempEnc = os.path.join(
                self.dir, "%s%s" % (self.nameStub + "-enc" + str(i), ".mp3")
            )
            junk, tempEncName = os.path.split(tempEnc)
            tempDec = os.path.join(
                self.dir, "%s%s" % (self.nameStub + "-dec" + str(i), ".wav")
            )
            junk, tempDecName = os.path.split(tempDec)

            postMp3 = encodeMp3(
                encodeSrcPath, None, br, "", "", "", "", tempEncName, 9
            )  # quality set to 9
...
def waterMark(src, spotWidth=70, spotDur=1.05, spotAmp=0.20):
    ...
    for q in range(0, totalSpots):
        ...
        # of spotWidth
        min = int(round((0.1 * spotWidth)))
        max = int(round((0.3 * spotWidth)))
...
def TestOld():
    ...
    a = ShardHarvest(src, dst, 0.1363636363636, 0.0136364)  # 300
