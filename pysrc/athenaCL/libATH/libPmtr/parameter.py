...
allPmtrObjs = rthmPmtrObjs + genPmtrObjs + textPmtrObjs + clonePmtrObjs + filterPmtrObjs
...
def pmtrTypeParser(typeName, libName="genPmtrObjs"):
    ...
    raise error.ParameterObjectSyntaxError(
        "no parameter named %r in %s" % (usrStr, pmtrLibTitle(libName))
    )
...
class Test(unittest.TestCase):
    ...
    def testGaRhythm(self):
        obj = factory(
            ("gaRhythm", ((3, 1), (4, 1), (5, 1, 0), (6, 1, 1))), "rthmPmtrObjs"
        )
    ...
    def testLoop(self):
        obj = factory(
            ("loop", ((3, 1), (4, 1), (5, 1, 0), (6, 1, 1)), "oc"), "rthmPmtrObjs"
        )
        self._parameterRunner(obj, 10)

        obj = factory(
            ("loop", ((3, 1), (4, 1), (5, 1, 0), (6, 1, 1)), "rp"), "rthmPmtrObjs"
        )
        self._parameterRunner(obj, 10)

        obj = factory(
            ("loop", ((3, 1), (4, 1), (5, 1, 0), (6, 1, 1)), "oo"), "rthmPmtrObjs"
        )
        self._parameterRunner(obj, 10)
    ...
    def testConvertSecond(self):
        obj = factory(("convertSecond", ("waveSine", "e", 30, 0, 1, 0)), "rthmPmtrObjs")
        self._parameterRunner(obj, 20)
    ...
    def testIterateGroup(self):
        ...
        args = [
            "iterateGroup",
            ("bg", "oc", [3, 8, 1, 7]),
            ("iw", (("bg", "oc", [1, 3, 5]), ("bg", "rc", [-1, -2])), ("c", 1), "rw"),
        ]
        self._parameterRunner(factory(args), 20)
    ...
    def testRandomExponential(self):
        args = ("randomExponential", 0.5, 20, 2000)
        self._parameterRunner(factory(args), 30)

    def testRandomInverseExponential(self):
        args = ("randomInverseExponential", 0.5, 20, 2000)
        self._parameterRunner(factory(args), 30)

    def testRandomBilateralExponential(self):
        args = ("randomBilateralExponential", 0.5, 20, 2000)
        self._parameterRunner(factory(args), 30)

    # -----------------------------------------------------------------------||--
    def testRandomGauss(self):
        args = ("randomGauss", 0.5, 1, 1, 9)
        self._parameterRunner(factory(args), 30)

    def testRandomCauchy(self):
        args = ("randomCauchy", 0.1, 0.5, 1, 9)
        self._parameterRunner(factory(args), 30)

    def testRandomBeta(self):
        args = ("randomBeta", 0.1, 0.1, 1, 9)
        self._parameterRunner(factory(args), 30)

    def testRandomWeibull(self):
        args = ("randomWeibull", 0.5, 2.0, 1, 9)
        self._parameterRunner(factory(args), 30)
    ...
    def testGrammarTerminus(self):
        args = ("gt", "a{.2}b{.5}c{.8}d{0}@a{ba}b{bc}c{cd}d{ac}@a", 10, "oc")
        self._parameterRunner(factory(args), 20)

    def testFeedbackModelLibrary(self):
        args = ["fml", "cc", ("bg", "rc", (1, 3)), ("c", 0.9), 0, 1]
        self._parameterRunner(factory(args), 20)

    # -----------------------------------------------------------------------||--
    def testBreakPointLinear(self):
        args = ("breakPointLinear", "e", "l", ((0, 0), (5, 1), (10, 0.5)))
        self._parameterRunner(factory(args), 20)

    def testBreakPointPower(self):
        args = ("breakPointPower", "e", "l", ((0, 0), (5, 1), (10, 0.5)), 2)
        self._parameterRunner(factory(args), 20)
