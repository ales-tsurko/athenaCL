...
class _Wave(basePmtr.Parameter):
    ...
    def repr(self, format=""):
        ...
        return "%s, %s, (%s), %s, (%s), (%s)" % (
            ...
            self.step,
            spcStr,
            typeset.anyDataToStr(self.phase),
            minStr,
            maxStr,
        )
...
class WaveSine(_Wave):
    def __init__(self, args, refDict):
        ...
        self.argDemos.append(["e", 20, 0, 0, ("ws", "e", 60, 0.25, 0.25, 1)])
...
class WaveCosine(_Wave):
    def __init__(self, args, refDict):
        ...
        self.argDemos.append(
            ["e", 40, 0, ("wp", "e", 20, 0, 1, 0.5), ("a", 0, ("c", 0.01))]
        )
...
class WaveSawUp(_Wave):
    def __init__(self, args, refDict):
        ...
        self.argDemos.append(["e", 20, 0, ("wpd", "e", 40, 0, 1.5, 1, 0.5), 0])
    ...
    def __init__(self, args, refDict):
        ...
        self.argDemos.append(
            [
                ...
                20,
                0,
                ("wpu", "e", 120, 0, 1.5, 0.5, 1),
                ("wpd", "e", 40, 0.25, 1.5, 0.5, 0),
            ]
        )
...
class WavePulse(_Wave):
    def __init__(self, args, refDict):
        ...
        self.argDemos.append(
            ["e", 10, 0, ("a", 0, ("ws", "e", 30, 0.75, -0.01, 0.03)), 0.5]
        )
...
class WaveTriangle(_Wave):
    def __init__(self, args, refDict):
        ...
        self.argDemos.append(["e", 30, 0, ("ru", 0, 0.3), ("ru", 0.7, 1)])
...
class _WaveHalfPeriod(_Wave):
    ...
    def __call__(self, t, refDict=None):
        ...
        if self.currentTimeRange == None or (
            tLocal < self.currentTimeRange[0] or tLocal >= self.currentTimeRange[1]
        ):
            ...
            self.currentTimeRange = [tLocal, tLocal + (self.spc * 0.5)]

            # flip flop phase shift to get second half of wave
            if self.phaseShift == None or self.phaseShift == 0.5:
                self.phaseShift = 0
            else:
                self.phaseShift = 0.5

        try:
            fq = 1.0 / self.spc  # convert spc to frequency
        except ZeroDivisionError:
            fq = 1  # override
            # raise error.ParameterObjectSyntaxError, 'spc/epc must be greater than zero.'

        rawPosition = self.obj(tLocal + self.timeShift, fq, phase0=self.phaseShift)
        # environment.printDebug(['current fq', fq, 'raw position', rawPosition])

        self.currentValue = unit.denorm(
            rawPosition,  # only use tLocal
            self.minObj(t, refDict),
            self.maxObj(t, refDict),
        )
        # increment after value returned
...
class WaveHalfPeriodPulse(_WaveHalfPeriod):
    def __init__(self, args, refDict):
        ...
        self.argDemos.append(
            ["e", ("bg", "rc", (15, 10, 5, 8, 2)), 0, 0, ("ls", "e", 100, 0.3, 1)]
        )
        self.obj = oscillator.Square(None, self.phase)  # omde object
...
class WavePowerUp(_WaveExponential):
    def __init__(self, args, refDict):
        ...
        self.argDemos.append(["e", 40, 0, 2, ("ru", 0, ("a", 0, ("c", 0.005))), 1])
...
class _Random(basePmtr.Parameter):
    ...
    def __call__(self, t, refDict=None):
        self.currentValue = unit.denorm(
            self.obj(), self.minObj(t, refDict), self.maxObj(t, refDict)
        )
        return self.currentValue
...
class RandomUniform(_Random):
    def __init__(self, args, refDict):
        ...
        self.argDemos = [
            [("ws", "e", 60, 0, 0.5, 0), ("ws", "e", 40, 0.25, 1, 0.5)],
        ]
...
class _RandomOneArg(basePmtr.Parameter):
    ...
    def repr(self, format=""):
        ...
        return "%s, %s, (%s), (%s)" % (
            self.type,
            typeset.anyDataToStr(self.lambd),
            minStr,
            maxStr,
        )
    ...
    def __call__(self, t, refDict=None):  # needs time
        self.currentValue = unit.denorm(
            self.obj(t), self.minObj(t, refDict), self.maxObj(t, refDict)
        )
        return self.currentValue
...
class RandomBilateralExponential(_RandomOneArg):
    def __init__(self, args, refDict):
        ...
        self.argDemos = [
            [10, 0, 1],
            [20, 0, ("bpp", "e", "l", ((0, 1), (40, 0.6), (80, 1)), 2)],
        ]
...
class _RandomTwoArg(basePmtr.Parameter):
    ...
    def repr(self, format=""):
        ...
        return "%s, %s, %s, (%s), (%s)" % (
            self.type,
            typeset.anyDataToStr(self.argA),
            typeset.anyDataToStr(self.argB),
            minStr,
            maxStr,
        )
...
class RandomGauss(_RandomTwoArg):
    def __init__(self, args, refDict):
        ...
        self.argDefaults = [0.5, 0.1, 0, 1]
        self.argDemos = [
            [0.5, 0.5, ("ws", "e", 120, 0.25, 0, 0.5), ("ws", "e", 120, 0.5, 1, 0.5)],
        ]
...
class RandomCauchy(_RandomTwoArg):
    def __init__(self, args, refDict):
        ...
        self.argDefaults = [0.1, 0.5, 0, 1]
        self.argDemos = [
            [0.1, 0.1, 1, ("bpp", "e", "l", ((0, 0), (120, 0.3)), 2)],
            [0.1, 0.9, 0, ("bpp", "e", "l", ((0, 1), (120, 0.3)), 2)],
        ]
...
class RandomWeibull(_RandomTwoArg):
    def __init__(self, args, refDict):
        ...
        self.argDefaults = [0.5, 2.0, 0, 1]
        self.argDemos = [[0.9, 0.1, 0, 1], [0.1, 0.9, ("ws", "e", 240, 0, 0, 0.4), 1]]
...
class _BreakPoint(basePmtr.Parameter):
    ...
    def _scrubPoints(self):
        ...
        if len(rawPoints) == 0:
            ...
            if drawer.isNum(rawPoints[0]) and drawer.isNum(
                rawPoints[1]
            ):  # shuld be a list
    ...
    def _setObj(self):
        ...
        if self.type == "breakPointLinear":
            ...
            self.obj = bpf.PowerSegment(self.points, exp=self.exp, periodic=loopArg)
...
class BreakPointLinear(_BreakPoint):
    def __init__(self, args, refDict):
        ...
        self.argDemos = [
            ["e", "s", ((12, 0.3), (18, 0.9), (24, 0.2), (48, 0.6))],
            [
                "e",
                "l",
                (
...
class BreakPointPower(_BreakPoint):
    def __init__(self, args, refDict):
        ...
        self.argDemos = [
            ...
            ["e", "s", ((12, 0.3), (18, 0.9), (24, 0.8), (48, 0.2)), -4],
        ]
...
class _BreakGraph(basePmtr.Parameter):
    def __init__(self, args, refDict):
        ...
        self.type = None  # assigned in subclass
        # 4th arg here is optional
        self.argTypes = [["int", "str"], ["int", "str"], "list", "list", "int", "num"]
        self.argDefaults = [
            "e",
            "l",
            ["a", 0, ("bg", "rp", [1, 3, 9])],
            ["bg", "rc", (0, 0.25, 0.5, 0.75, 1)],
            60,
            -1.5,
        ]
        # check raw arguments for number, type
...
class BreakGraphLinear(_BreakGraph):
    def __init__(self, args, refDict):
        ...
        self.argNames = [
            ...
            "parameterObject: y point Generator",
            "pointCount",
        ]
        self.argDemos = []
...
class LineSegment(basePmtr.Parameter):
    def __init__(self, args, refDict):
        ...
        self.argNames = ["stepString", "parameterObject: secPerCycle", "min", "max"]
        self.argDemos = [
    ...
    def _genPoints(self, refDict, t):
        ...
        if lineSpan == 0:
            raise error.ParameterObjectSyntaxError(
                "seconds per cycle parameter object returns zero."
            )
...
class EnvelopeGeneratorUnit(basePmtr.Parameter):
    def __init__(self, args, refDict):
        ...
        self.argDefaults = ["l", 100, ["c", 40], ["c", 0.4], ["c", 0.2], 0, 1]  # dur
        self.argDemos = [
            [
                "l",
                100,
