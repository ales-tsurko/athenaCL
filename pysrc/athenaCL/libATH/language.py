...
Apple, Macintosh, Mac OS, and QuickTime are trademarks or registered trademarks of Apple Computer, Inc. Finale is a trademark of MakeMusic! Inc. Java is a trademark of Sun Microsystems. Linux is a trademark of Linus Torvalds. Max/MSP is a trademark of Cycling '74. Microsoft Windows and Visual Basic are trademarks or registered trademarks of Microsoft, Inc. PDF and PostScript are trademarks of Adobe, Inc. Sibelius is a trademark of Sibelius Software Ltd. SourceForge.net is a trademark of VA Software Corporation. UNIX is a trademark of The Open Group."""
    ...
    msgBadAthObj = (
        TAB
        + "there is a problem with this AthenaObject file. make sure an AthenaObject has been selected, then try (1) opening the file in a text editor and re-saving the file with the appropriate line endings or (2) restart athenaCL and try again.\n"
    )
    ...
    msgFileError = (
        "a bad file path has been selected; try to write files in a better location.\n"
    )
    ...
    msgDlgBadFileNameStart = (
        TAB + ERROR + ' a file name cannot start with "%s". try again.\n'
    )
    msgDlgBadFileNameChar = (
        TAB + ERROR + ' a file name cannot contain "%s". try again.\n'
    )
    ...
    msgDocAdditionalHelp = (
        'for help with a command, enter "?" followed by the command.\n'
    )
    ...
    msgSCcentRangeError = (
        TAB
        + "a percentage range must be entered as two values separated by a comma. try again.\n"
    )
    msgSCcentValueError = (
        TAB
        + "percentage values can be either decimals (0 to 1) or percentages (0 to 100).\n"
    )
    msgSCtni = "TnI"
    msgSCtn = "Tn"
    ...
    msgSCenterFindMethod = (
        "select a find method: name, z-relation, or super-sets? (n, z, s):"
    )
    ...
    msgMCgetBadVlPairs = (
        TAB + "cannot yet handle vl pairs between boundaries of different sizes."
    )
    msgMCgetBadVlParse = TAB + "unable to parse vl pairs.\n"
    msgMCgetBadVlLimit = TAB + "vl pairs can describe at most 6 voices."

    msgMCbadMapRange = (
        TAB + "incorrect map range. enter a start map, a comma, and an end map.\n"
    )
    msgMCbadPosition = TAB + "incorrect position entry. try again.\n"
    msgMCbadRankRange = (
        TAB + "incorrect rank range. enter a start rank, a comma, and an end rank.\n"
    )
    msgMCbadVoiceRange = (
        TAB + "incorrect entry. enter (sourceNumber, destinationNumber).\n"
    )
    msgMCbadVoiceValue = (
        TAB
        + "the number of voices in a source or destination must be between 1 and 6.\n"
    )
    msgMCbadRawMapFormat = TAB + "incorrect raw map format.\n"
    msgMCbadVoiceFormat = TAB + "incorrect voice-leading format.\n"
    msgMCbadMapChoice = (
        TAB + "between %r and %r voices you must choose a MC between 1 and %r.\n"
    )
    ...
    msgMCenterRankMethod = (
        "enter ranking method: Smoothness, Uniformity or Balance? (s, u, or b):"
    )
    ...
    msgMCenterMapRange = (
        "there are %i maps in this ranking. enter a range: (beginRank,endRank):"
    )
    msgMCenterMapSizeRange = (
        "there are %i maps in this size group. enter a range: (beginMap,endMap):"
    )
    msgMCgetMapMenu = "enter a map, map class, or vl pairs:"
    msgMCminDisplVal = (
        "minimum displacement values for all sets between cardinality %i and %i\n"
    )
    ...
    msgPIbadDuration = (
        TAB + "enter a list of %s values greater than zero, separated by commas.\n"
    )
    ...
    msgTIunmuteFirst = (
        'unmute a texture or a clone first: enter "TImute" or "TCmute".\n'
    )
    ...
    msgTInoAuxEdit = (
        "TIe %s" % ERROR + " this instrument has no auxiliary parameter fields\n"
    )
    msgTInoTextEdit = (
        "TIe %s" % ERROR + " this texture has no static texture parameters\n"
    )
    msgTInoDynEdit = (
        "TIe %s" % ERROR + " this texture has no dynamic texture parameters\n"
    )
    ...
    msgTImodePolyChoose = (
        TAB + "current Polyphony Mode: %s. enter new mode (set, part):"
    )
    ...
    msgTCbadTimeEntry = (
        TAB
        + "a TextureClone cannot start before time 0. enter a more positive timeShift.\n"
    )
    msgTCbadPmtrName = (
        'no such parameter exists: enter "TCls" to display all parameters.\n'
    )
    msgTCbadEdit = "bad TC edit data. no changes have been made.\n"
    msgTCnoAuxEdit = (
        "TCe %s" % ERROR + " this instrument has no auxiliary parameter fields\n"
    )
    ...
    msgEMnoInstrument = (
        TAB + 'no such instrument available, enter "?" to view all instruments.\n'
    )
    ...
    msgELfileMoved = (
        'the file (%s) cannot be found. enter "ELn" to create new EventList files.\n'
    )
    ...
    msgELrenderError = (
        "audio rendering failed (%s). check audio and csound preferences.\n"
    )
    msgELrenderInit = "audio rendering initiated: %s\n"
    msgELrNotAvailable = "EventList rendering not available under EventMode: %s\n"
    msgELaudioMoved = (
        'audio file (%s) cannot be found. enter "ELr" to render an audio file.\n'
    )
    ...
    docTmInterpolateLine = (
        "This TextureModule interpolates parameters between events generated under a linear monophonic context. %s"
        % (_interpolation)
    )

    docTmInterpolateFill = (
        "This TextureModule interpolates parameters between events generated under a non-linear monophonic context. %s"
        % (_interpolation)
    )
    ...
    # unit scaling
    _normalizedList = (
        "The resulting list of values is normalized within the unit interval."
    )
    ...
    _waveValuePeriod = (
        "between 0 and 1 at a rate given in either time or events per period."
    )
    ...
    _waveCommon = (
        "%s %s The phase argument is specified as a value between 0 and 1. Note: conventional cycles per second (cps or Hz) are not used for frequency."
        % (_waveSecondsPerCycle, _scaleCommon)
    )
    ...
    _waveHalfPeriodCommon = (
        "%s %s The phase argument is specified as a value between 0 and 1. Half-period oscillators update seconds/event per cycle only once per half-period, permitting smooth transitons between diverse half-period segments. Note: conventional cycles per second (cps or Hz) are not used for frequency."
        % (_waveSecondsPerCycleHalfPeriod, _scaleCommon)
    )
    ...
    _divisionByZeroGenerator = (
        "Division by zero, if encountered, returns the value of the first Generator."
    )
    ...
    docPoBg = (
        "Chooses values from a user-supplied list (valueList). Values can be strings or numbers. %s"
        % _selectionStr
    )
    ...
    docPoBf = (
        "Chooses values from a ParameterObject generated list of values. The number of values generated is determined by the valueCount integer. Valuse are generated only at initialization. %s"
        % _selectionStr
    )
    ...
    docPoSl = (
        "Produces a Xenakis sieve as a raw, variable format sieve segment list. A z is defined by the range of integers from zMin to zMax. Depending on format type, the resulting segment can be given as an integer, width, unit, or binary segment. %s"
        % _selectionStr
    )
    ...
    docPoSf = (
        "%s Values produced with the fill value Generator ParameterObject are funneled through this sieve: given a fill value, the nearest sieve value is selected and returned. Note: the fill value ParameterObject min and max should be set to 0 and 1."
        % (_sieveHeader)
    )

    docPoLp = (
        "Produces a segment of prime (pseudoprime) integers defined by a positive or negative start value and a length. Depending on format type, the resulting segment can be given as an integer, width, unit, or binary segment. %s"
        % _selectionStr
    )

    docPoVp = (
        "Produces a segment of prime (pseudoprime) integers defined by a positive or negative start value and a length. %s %s %s"
        % (_normalizedList, _selectionStr, _scaleAfter)
    )

    docPoCl = (
        "Produces values from a one-dimensional cellular automata table. %s %s %s"
        % (_caHeader, _tableExtract, _selectionStr)
    )

    docPoCv = (
        "Produces values from a one-dimensional cellular automata table scaled within dynamic min and max values. %s %s %s %s"
        % (_caHeader, _tableExtract, _selectionStr, _scaleAfter)
    )

    docPoGt = (
        "Produces values from a one-dimensional string rewrite rule, or Lindenmayer-system generative grammar. The terminus, or final result of the number of generations of values specifed by the stepCount parameter, is used to produce a list of defined values. %s"
        % (_selectionStr)
    )

    docPoFml = "A model of a feedback system made from discrete particles. %s" % (
        _scaleCommon
    )
    ...
    docPoM = (
        "Given values produced by two boundary ParameterObjects in parallel, the Generator ParameterObject value is fit within these values. %s"
        % (_maskFit)
    )

    docPoMr = (
        "Given values produced by two boundary ParameterObjects in parallel, the Generator ParameterObject value is fit outside of these values. %s"
        % (_maskFit)
    )

    docPoMs = (
        "Given values produced by two boundary ParameterObjects in parallel, the Generator ParameterObject value is scaled within these values. A collection of values created by the Generator ParameterObject are stored. %s %s %s"
        % (_normalizedList, _selectionStr, _scaleAfter)
    )
    ...
    _iterateWindow = (
        "A numeric value from a control ParameterObject is used to determine the selected ParameterObject behavior. A positive value (rounded to the nearest integer) will cause the selected ParameterObject to produce that many new values. After output of these values, a new ParameterObject is selected. A negative value (rounded to the nearest integer) will cause the selected ParameterObject to generate and discard that many values, and force the selection of a new ParameterObject. A value equal to 0 is treated as a bypass, and forces the selection of a new ParameterObject. ParameterObject selection is determined with a string argument for a selection method. Note: %s."
        % (_controlFailure)
    )

    docPoIw = (
        "Allows a ParameterObject, selected from a list of ParameterObjects, to generate values, to skip values (a number of values are generated and discarded), or to bypass value generation. %s"
        % (_iterateWindow)
    )

    docPoIrw = (
        "Allows a Rhythm ParameterObject, selected from a list of Rhythm ParameterObjects, to generate values, to skip values (a number of values are generated and discarded), or to bypass value generation. %s"
        % (_iterateWindow)
    )

    _iterateGroup = (
        "A numeric value from a control ParameterObject is used to determine the source ParameterObject behavior. A positive value (rounded to the nearest integer) will cause the value provided by the source ParameterObject to be repeated that many times. After output of these values, a new control value is generated. A negative value (rounded to the nearest integer) will cause that many number of values to be generated and discarded from the source ParameterObject, and force the selection of a new control value. A value of 0 is treated as a bypass, and forces the selection of a new control value. Note: %s."
        % (_controlFailure)
    )

    docPoIg = (
        "Allows the output of a source ParameterObject to be grouped (a value is held and repeated a certain number of times), to be skipped (a number of values are generated and discarded), or to be bypassed. %s"
        % _iterateGroup
    )

    docPoIrg = (
        "Allows the output of a source Rhythm ParameterObject to be grouped (a value is held and repeated a certain number of times), to be skipped (a number of values are generated and discarded), or to be bypassed. %s"
        % _iterateGroup
    )

    _iterateHold = (
        "%s A numeric value from a size ParameterObject is used to determine how many values are drawn from the source ParameterObject. A numeric value from a refresh count ParameterObject is used to determine how many events must pass before a new size value is drawn and the source ParameterObject is used to refill the stored list. A refresh value of zero, once encountered, will prohibit any further changes to the stored list. Note: if the size ParameterObject fails to produce a non-zero value for the first event, an alternative count value will be assigned."
        % (_selectionStr)
    )

    docPoIh = (
        "Allows a variable number of outputs from a source ParameterObject, collected and stored in a list, to be held and selected. %s"
        % (_iterateHold)
    )

    docPoIs = (
        "Allows a variable number of outputs from a source ParameterObject, collected and stored in a list, to be selected with values within the unit interval produced by an embedded ParameterObject. Values that exceed the unit interval are limited within the unit interval. %s"
        % (_iterateHold)
    )

    docPoIrh = (
        "Allows a variable number of outputs from a source Rhythm ParameterObject, collected and stored in a list, to be held and selected. %s"
        % (_iterateHold)
    )
    ...
    docPoMv = (
        "Produces values by means of a Markov transition string specification and a dynamic transition order generator. %s"
        % _dynamicMarkovOrder
    )

    docPoMga = (
        "Produces values by means of a Markov analysis of values provided by a source Generator ParameterObject; the analysis of these values is used with a dynamic transition order Generator to produce new values. The number of values drawn from the source Generator is specified with the valueCount argument. The maximum order of analysis is specified with the maxAnalysisOrder argument. %s"
        % _dynamicMarkovOrder
    )

    docPoOa = (
        "Adds the value of the first ParameterObject to the second ParameterObject."
    )

    docPoOs = "Subtracts the value of the second ParameterObject from the first ParameterObject."

    docPoOd = (
        "Divides the value of the first ParameterObject object by the second ParameterObject. %s"
        % _divisionByZeroGenerator
    )
    ...
    docPoWs = "Provides sinusoid oscillation %s %s" % (_waveValuePeriod, _waveCommon)
    docPoWc = "Provides cosinusoid oscillation %s %s" % (_waveValuePeriod, _waveCommon)
    ...
    docPoWp = "Provides a pulse (square) wave %s %s" % (_waveValuePeriod, _waveCommon)
    ...
    docPoWpd = "Provides a power down wave %s %s" % (_waveValuePeriod, _waveCommon)

    docPoWhps = "Provides half-period sinusoid oscillation %s %s" % (
        _waveValuePeriod,
        _waveHalfPeriodCommon,
    )
    docPoWhpc = "Provides half-period cosinusoid oscillation %s %s" % (
        _waveValuePeriod,
        _waveHalfPeriodCommon,
    )
    docPoWhpp = "Provides half-period pulse (square) wave oscillation %s %s" % (
        _waveValuePeriod,
        _waveHalfPeriodCommon,
    )
    docPoWhpt = "Provides half-period triangle wave oscillation %s %s" % (
        _waveValuePeriod,
        _waveHalfPeriodCommon,
    )

    docPoRu = (
        "Provides random numbers between 0 and 1 within an uniform distribution. %s Note: values are evenly distributed between min and max."
        % _scaleCommon
    )
    docPoRl = (
        "Provides random numbers between 0 and 1 within a linearly decreasing distribution. %s Note: values are distributed more strongly toward min."
        % _scaleCommon
    )
    docPoRil = (
        "Provides random numbers between 0 and 1 within a linearly increasing distribution. %s Note: values are distributed more strongly toward max."
        % _scaleCommon
    )
    docPoRt = (
        "Provides random numbers between 0 and 1 within a triangular distribution. %s Note: values are distributed more strongly toward the mean of min and max."
        % _scaleCommon
    )
    docPoRit = (
        "Provides random numbers between 0 and 1 within an inverse triangular distribution. %s Note: values are distributed more strongly away from the mean of min and max."
        % _scaleCommon
    )

    docPoRe = (
        "Provides random numbers between 0 and 1 within an exponential distribution. %s Lambda values should be greater than 0. Lambda values control the spread of values; larger values (such as 10) increase the probability of events near the minimum."
        % _scaleCommon
    )
    docPoRie = (
        "Provides random numbers between 0 and 1 within an inverse exponential distribution. Lambda values control the spread of values; larger values (such as 10) increase the probability of events near the maximum. %s"
        % _scaleCommon
    )
    docPoRbe = (
        "Provides random numbers between 0 and 1 within a bilateral exponential distribution. %s"
        % _scaleCommon
    )

    docPoRg = (
        "Provides random numbers between 0 and 1 within a Gaussian distribution. %s Note: suggested values: mu = 0.5, sigma = 0.1."
        % _scaleCommon
    )
    docPoRc = (
        "Provides random numbers between 0 and 1 within a Cauchy distribution. %s Note: suggested values: alpha = 0.1, mu = 0.5."
        % _scaleCommon
    )
    docPoRb = (
        "Provides random numbers between 0 and 1 within a Beta distribution. %s Alpha and beta values should be between 0 and 1; small alpha and beta values (such as 0.1) increase the probability of events at the boundaries."
        % _scaleCommon
    )
    docPoRw = (
        "Provides random numbers between 0 and 1 within a Weibull distribution. %s Note: suggested values: alpha = 0.5, beta = 2.0."
        % _scaleCommon
    )

    docPoEgt = (
        "Generates a sequence of dynamic envelopes with durations controlled by a Generator Parameter Object. %s. %s"
        % (_envelopeCommon, _scaleEnvelope)
    )
    ...
    docPoFs = (
        "Provides values derived from a contigous section of the Fibonacci series. A section is built from an initial value (start) and as many additional values as specified by the length argument. Negative length values reverse the direction of the series. %s %s %s"
        % (_normalizedList, _selectionStr, _scaleAfter)
    )
    ...
    docPoBpl = (
        "Provides a break-point function with linear interpolation %s" % _bpCommon
    )

    docPoBpp = (
        "Provides a break-point function with exponential interpolation %s The exponent argument may be any positive or negative numeric value."
        % _bpCommon
    )

    docPoBphc = (
        "Provides a break-point function with half-cosine interpolation %s" % _bpCommon
    )
    ...
    docPoBgl = (
        "Provides a dynamic break-point function with linear interpolation. %s"
        % _bgCommon
    )

    docPoBgp = (
        "Provides a dynamic break-point function with exponential interpolation. %s The exponent argument may be any positive or negative numeric value."
        % _bgCommon
    )

    docPoBghc = (
        "Provides a dynamic break-point function with half-cosine interpolation. %s The exponent argument may be any positive or negative numeric value."
        % _bgCommon
    )

    docPoBgf = (
        "Provides a dynamic break-point function without interpolation. %s" % _bgCommon
    )

    docPoLs = (
        "Provides a dynamic line segment created from three embedded Generator ParameterObjects. Start and end values, taken from min and max generators, are used to create a line segment spaning the time or event distance provided by the secPerCycle argument. %s"
        % (_waveSecondsPerCycle)
    )
    ...
    docPoSs = (
        "Given a list of file names (fileNameList), this Generator provides a complete file path to the file found within either the athenaCL/audio or the user-selected audio directory. %s"
        % _selectionStr
    )

    docPoAs = (
        "Given a list of file names (fileNameList), this Generator provides a complete file path to the file found within either the libATH/sadir or the user-selected sadir. %s"
        % _selectionStr
    )

    docPoDs = (
        'Within a user-provided directory (directoryFilePath) and all sub-directories, this Generator finds all files named with a file extension that matches the fileExtension argument, and collects these complete file paths into a list. %s Note: the fileExtension argument string may not include a leading period (for example, use "aif", not ".aif"); %s.'
        % (_selectionStr, _symLinks)
    )

    docPoCf = (
        "Given an absolute file path, a constant file path is returned as a string. Note: %s."
        % _symLinks
    )
    ...
    docPoCs = (
        "Allows the use of a Generator ParameterObject to create rhythm durations. Values from this ParameterObject are interpreted as equal Pulse duration and sustain values in seconds. Accent values are fixed at 1. Note: %s."
        % (_bpmIndependent)
    )

    docPoCst = (
        "Allows the use of three Generator ParameterObjects to directly specify duration, sustain, and accent values. Values for duration and sustain are interpreted as values in seconds. Accent values must be between 0 and 1, where 0 is a measured silence and 1 is a fully sounding event. Note: %s."
        % (_bpmIndependent)
    )

    docPoPs = (
        "%s This sieve, as a binary or width segment, is interpreted as a pulse list. The length of each pulse and the presence of rests are determined by the user-provided Pulse object and the articulationString argument. An articulationString of 'attack' creates durations equal to the provided Pulse for every non-zero binary sieve segment value; an articulationString of 'sustain' creates durations equal to the Pulse times the sieve segment width, or the duration of all following rests until the next Pulse. %s"
        % (_sieveHeader, _selectionStr)
    )

    docPoRs = (
        "%s The resulting binary sieve segment is used to filter any non-rest Pulse sequence generated by a Rhythm ParameterObject. The sieve is interpreted as a mask upon the ordered positions of the generated list of Pulses, where a sieve value retains the Pulse at the corresponding position, and all other Pulses are converted to rests. Note: any rests in the generated Pulse sequence will be converted to non-rests before sieve filtering."
        % (_sieveHeader)
    )

    docPoMp = (
        "Produces Pulse sequences by means of a Markov transition string specification and a dynamic transition order generator. The Markov transition string must define symbols that specify valid Pulses. %s"
        % _dynamicMarkovOrder
    )

    docPoMra = (
        "Produces Pulse sequences by means of a Markov analysis of a rhythm provided by a source Rhythm Generator ParameterObject; the analysis of these values is used with a dynamic transition order Generator to produce new values. The number of values drawn from the source Rhythm Generator is specified with the pulseCount argument. The maximum order of analysis is specified with the maxAnalysisOrder argument. %s"
        % _dynamicMarkovOrder
    )
    ...
    docPoFd = (
        "Each input value is divided by a value produced by a user-supplied ParameterObject. %s"
        % _divisionByZeroFilter
    )
    ...
    docPoMf = (
        "Each input value is fit between values provided by two boundary Generator ParameterObjects. %s"
        % (_maskFit)
    )

    docPoMsf = "Each input value is collected into a list. %s %s %s" % (
        _normalizedList,
        _selectionStr,
        _scaleAfter,
    )
    ...
    docPoSet = (
        "Controls if all event start times are shifted to align with frame divisions."
    )
    ...
    docPoTec = (
        "Selects the total number of events generated within the Texture time range."
    )
    ...
    docPoLfd = (
        "Toggle between selection of frame duration values per frame or per event."
    )

    docPoPic = (
        "Controls if all non-duration parameter values are interpolated between events."
    )
