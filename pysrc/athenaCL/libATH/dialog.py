...
def _macPutFile(prompt="save a file", defaultName="test.txt", defaultDir=None):
    ...
    if carbon:
        ...
        path = EasyDialogs.AskFileForSave(message=prompt, savedFileName=defaultName)
        if path == None:
            return "", 0
        else:
            return path, 1
...
def _textGetDirectory(prompt="select a directory: ", sampleDir="", termObj=None):
...
def promptGetDir(
    prompt="select a directory:", sampleDir="", dlgVisualMethod=None, termObj=None
):
    ...
    if sampleDir != "":
        pass  # used as default dir selection
    elif drawer.getcwd() != None:
    ...
    if dlgVisualMethod[:2] == "tk" and TK == 1:
        ...
        options = {"filetypes": [("directory", "*")], "title": prompt, "parent": tkTemp}
...
def promptPutFile(
    ...
    if dlgVisualMethod[:2] == "tk" and TK == 1:
        ...
        options = {"filetypes": [("all files", "*")], "title": prompt, "parent": tkTemp}
    ...
    try:
        msg, ok = _textPutFile(
            prompt, defaultName, defaultDir, extension, dlgVisualMethod, termObj
        )
    except (OSError, IOError):  # catch file errors, or dirs called tt dont exist
        msg = ""  # this is usually the path itself
        ok = 0
