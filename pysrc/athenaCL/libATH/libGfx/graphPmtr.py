...
class _MapCanvas:
    ...
    def _genGraph(self, dataList, x=0, y=0, title=""):
        ...
        dataObj = imageTools.ProcessGraphCoordData(
            dataLib, self.xRez, self.yRez, self.dataSigDig, self.axisSigDig
        )
        ...
        gObj.setMargin(
            self.lMarginSize, self.rMarginSize, self.tMarginSize, self.bMarginSize
        )  # l, r, t, b
        gObj.setIconSize(
            self.xIconSize, self.yIconSize, self.xIconMargin, self.yIconMargin
        )
        # x, y, xMargin, yMargin
...
class TPmapCanvas(_MapCanvas):
    ...
    def __init__(self, ao, srcList, srcFmt="pg", events=240, fmt="tk", master=None):
        _MapCanvas.__init__(self)
        self.c = fmt
        ...
        # create canvas
        self.c = imageTools.Canvas(
            fmt, self.winWIDTH, self.winHEIGHT, self.COLORbgAbs, "TPmap", master
        )
        # draw all parameters
