...
# -----------------------------------------------------------------||||||||||||--
def writeUnixLauncher(
    pathLaunchScript, pathShellScript, optInstallTool=0, sudoFound=None
):
...
if _fpPackageDir != fpPackageDir:
    environment.printWarn(
        "cwd (%s) is not the athenaCL src directory (%s)" % (_fpPackageDir, fpSrcDir)
    )
...
if os.name == "posix":  # create launch script
    ...
    if optWriteLauncher:  # will determine optInstallTool
        ...
        writeUnixLauncher(pathLaunchScript, pathShellScript, optInstallTool, sudoFound)
    if optRemoveMan:
        removeMan(sudoFound)
    if optInstallMan:
