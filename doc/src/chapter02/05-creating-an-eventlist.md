...
other EventOutput format file names are derived from this source .xml file path.
If EventOutput xmlAthenaObject is active, an XML AthenaObject file will be
...
immediately: athenaCL has its own player. To hear the file produced by `ELn`,
enter the command `ELh`, for EventList hear, and a player appears in the log:
...
![The player that ELh puts in the log](../images/player.png)
...
Using the `ELh` command to listen to this EventList, the user should identify
that although "test01" and "test02" are closely related, each musical fragment,
due to algorithmic variation, has differences.
