...
fontLibPil = {
    ...
    "lub": ("lubB", "luBIS", "lubI", "luBS", "lubR", "luIS", "luRS", "lutBS", "lutRS"),
...
class GridGraphic:
    ...
    def scale(self, xMult, yMult):
        self.imageObj = self.imageObj.resize((self.noCols * xMult, self.noRows * yMult))
...
class GridText(GridGraphic):
    def __init__(
        ...
        obj = fontLibrary.FontBitMap(
            msg, font, kernEqual, kernBit, vertBit, charWrapWidth
        )
        gridData = obj.encode()
...
class TkCanvas(_CanvasBase):
    ...
    def __init__(self, w, h, bg, name, master):
        ...
        self.frame = tkinter.Frame(
            self.master, width=self.w, height=self.h, bd=0, bg=self.bg, relief="flat"
        )
    ...
    def rectangle(self, x1, y1, x2, y2, fill, outline, width=1):
        ...
        self.c.create_rectangle(
            x1, y1, x2 + 1, y2 + 1, fill=fill, outline=outline, width=width
        )
    ...
    def bitmap(self, xShift, yShift, gridData, color=None):
        ...
        if os.name == "mac":
            ...
            xBuf = 1  # win tk requires extra pixel
        ...
        for row in gridData:
            for col in row:
                ...
                if colorUse != None:
                    self.c.create_line(
                        (xShift + x, yShift + y, xShift + x + xBuf, yShift + y),
                        fill=colorUse,
                        width=1,
                        capstyle="round",
                    )
...
class PilCanvas(_CanvasBase):
    def __init__(self, w, h, bg, name, transp, subFmt):
        ...
        if self.bg == None or transp != None:
            pass
        else:
    ...
    def rectangle(self, x1, y1, x2, y2, fill, outline, width=1):
        ...
        if width > 1:  # draw rectangles inside to create width on outline
            for i in range(1, width):  # one pixel is laready drawin
                self.c.rectangle(
                    (x1 + i, y1 + i, x2 - i, y2 - i), fill=None, outline=outline
                )
...
%%EOF"""
    ...
    def _convertCoordArc(self, x1, y1, x2, y2):
        ...
        radius = (x2 - x1) * 0.5  # assume its a circle
    ...
    def rectangle(self, x1, y1, x2, y2, fill=None, outline=None, width=1):
        ...
        if outline == None:
            ...
            strOutline = "%s setlinewidth %s %s %s setrgbcolor stroke" % (
                width,
                r,
                g,
                b,
        ...
        msg.append(
            """
/%s { newpath %s %s moveto %s %s lineto %s %s lineto %s %s lineto closepath gsave %s grestore gsave %s grestore} def """
            % (
                strDefName,
                ll[0],
                ll[1],
...
"""
    ...
    def line(self, x1, y1, x2, y2, fill, width=0.5):
...
"""
            % (strDefName, pairL[0], pairL[1], pairR[0], pairR[1], strFill)
        )
        msg.append(
            """
gsave %s 1 1 scale grestore
    ...
    def _charFilter(self, usrStr):
        ...
        for char in usrStr:
            if char in [
                "(",
                ")",
            ]:  # not sure if back-slash should be here
                msg.append("\%s" % char)  # escape
            else:
                msg.append(char)
        return "".join(msg)

    def fontText(self, x, y, anchor="nw", text="test", font=None, fill="#342396"):
        ...
        fontHeight = round((int(fontSize) * 0.55), 1)
        ...
        if anchor == "sw":  # default is lower left
            ...
            strMove = "%s (%s) stringwidth pop sub %s moveto" % (x, text, y)
        elif anchor == "sc":  # pop removes y coord
            strMove = "%s (%s) stringwidth pop 2 div sub %s moveto" % (x, text, y)
        elif anchor == "cw":
            strMove = "%s %s %s 2 div sub moveto" % (x, y, fontHeight)
...
/%s { newpath %s %s %s %s %s arc closepath gsave %s grestore gsave %s grestore} def """
            % (strDefName, xCenter, yCenter, radius, 0, 360, strFill, strOutline)
        )
        msg.append(
            """
gsave %s 1 1 scale grestore
...
class ProcessGraphCoordData:
    ...
    def _genAxis(self, min, max, steps, side):
        ...
        if sigDig != 0:
            if drawer.isInt(min):
                ...
                rMin = round(min, sigDig)
            if drawer.isInt(max):
                rMax = max
            else:
                rMax = round(max, sigDig)
    ...
    def _findAxis(self):
        ...
        self.yAxisLib = {}
        for key in list(self.dataLib.keys()):
            self.xAxisLib[key] = self._genAxis(
                self.xRangeLib[key][0], self.xRangeLib[key][1], self.xRez, "x"
            )
            self.yAxisLib[key] = self._genAxis(
                self.yRangeLib[key][0], self.yRangeLib[key][1], self.yRez, "y"
            )
...
class _GraphBase:
    ...
    def update(self, canvas="png"):
        ...
        if self.canvas != None:
            self.canvas.rectangle(
                self.xOrigin,
                self.yOrigin,
                self.xOrigin + self.wTotal - 1,
                self.yOrigin + self.hTotal - 1,
                self.colorBkg,
                None,
            )
    ...
    def setTitle(self, title, titlePos="br", titleGutterVert=2, titleGutterHorz=None):
        self.title = title
    ...
    def setColors(self, bkg, grid, tick, margin, units, label, title, bitColorDict):
        self.colorBkg = bkg
        self.colorGrid = grid
        self.colorTick = tick
    ...
    def _iconA(self, args):
        ...
        if self.hIcon == 1:  # draw a line
            self.canvas.rectangle(
                x, y, x + self.wIcon - 1, y, self.bitColorDict[1], None, 0
            )
    ...
    def _iconB(self, args):
        x, y = args
        self.canvas.rectangle(
            x, y, x + self.wIcon - 1, y + self.hIcon - 1, self.bitColorDict[2], None, 1
        )

    def _iconC(self, args):
        x, y = args
        self.canvas.rectangle(
            x, y, x + self.wIcon - 1, y + self.hIcon - 1, self.bitColorDict[3], None, 0
        )
    ...
    def _drawMargin(self):
        ...
        if self.tMargin != 0:
            self.canvas.rectangle(
                self.xOrigin,
                self.yOrigin,
                self.wTotal + self.xOrigin - 1,
                self.yOrigin + self.tMargin - 1,
                self.colorMargin,
                None,
                0,
            )
        ...
        if self.rMargin != 0:
            self.canvas.rectangle(
                (self.xOrigin + self.lMargin + self.wGraph),
                self.yOrigin,
                self.wTotal + self.xOrigin - 1,
                self.yOrigin + self.hTotal - 1,
                self.colorMargin,
    ...
    def _drawTick(self):
        for x, y, yPos, i in self._yGraphPosList():
            ...
            if self.lTickKey != []:  # not empty
                ...
                if pix != 0:
                    self.canvas.line(x - pix, yPos, x - 1, yPos, self.colorTick)
            # do right
        ...
        for x, y, xPos, j in self._xGraphPosList():
            ...
            if self.tTickKey != []:  # not empty
                ...
                if pix != 0:
                    self.canvas.line(xPos, y - pix, xPos, y, self.colorTick)
    ...
    def _drawTitle(self):
        ...
        self.canvas.gridText(
            xPos, yPos, anchor, self.title, self.font, self.colorTitle, self.kernBit
        )
    ...
    def _xGridToPixel(self, xRaw):
        ...
        x = self.graphOrigin[0] + ((xRaw + 1) * self.wIconGutter) + (xRaw * self.wIcon)
        return x
    ...
    def _yGridToPixel(self, yRaw):
        ...
        # always a margin (at 0) so add 1
        # no width at zero, so leave as is
        y = self.graphOrigin[1] + ((yRaw + 1) * self.hIconGutter) + (yRaw * self.hIcon)
        return y
...
class NumericalPointGraph(_GraphBase):
    ...
    def __init__(self, dataObj=None, origin=(0, 0)):
        ...
        _GraphBase.__init__(
            self, dataObj.xRez, dataObj.yRez, dataObj.getPlotLib(), origin
        )
        # set preferences
    ...
    def _iconA(self, args):
        ...
        # x and y do not include margin; are icon's upper left and right corner
        self.canvas.rectangle(
            x, y, x + self.wIcon - 1, y + self.hIcon - 1, self.bitColorDict[1], None, 0
        )
    ...
    def _iconB(self, args):
        ...
        # x and y do not include margin; are icon's upper left and right corner
        self.canvas.rectangle(
            x, y, x + self.wIcon - 1, y + self.hIcon - 1, self.bitColorDict[2], None, 0
        )
...
class NumericalBarGraph(_GraphBase):
    ...
    def _iconA(self, args):
        ...
        # x2+self.wIcon-1
        self.canvas.rectangle(
            x1, y1, x2 - 1, y2 + self.hIcon - 1, self.bitColorDict[1], None, 0
        )

    def _iconB(self, args):
        x1, y1, x2, y2 = args
        self.canvas.rectangle(
            x1, y1, x2 - 1, y2 + self.hIcon - 1, self.bitColorDict[2], None, 0
        )
...
class TextGraph(_GraphBase):
    ...
    def __init__(
        ...
        if msg == None:
            msg = "algorithmic.net"
        # process data
        obj = fontLibrary.FontBitMap(msg, font, 0, kernBit, vertBit, charWrapWidth)
        gridData = obj.encode()
    ...
    def _iconA(self, args):
        ...
        # corner
        self.canvas.rectangle(
            x, y, x + self.wIcon - 1, y + self.hIcon - 1, self.bitColorDict[1], None, 1
        )
...
class TestOld:
    ...
    def testText(self):
        ...
        a = GridText(
            "testing a long sample of text!", fg, bg, "micro", 6, None, 2, 2, 1, 10
        )
        a.show()
    ...
    def testCanvas(self):
        ...
        for fmt in fmtList:
            ...
            a.line(0, 100, 200, 100, "#999999", 0.25)
            a.line(100, 0, 100, 200, "#999999", 0.25)
            # problem case
            a.line(0, 200, 200, 0, "#999999", 0.25)

            a.gridText(0, 0, "nw", "testing some large text", "micro", "#33ff99", 2)
