...
def convertBoolCancel(usrData):
    ...
    # off
    if (
        usrStr in ["0", lang.OFF.lower(), lang.NO.lower(), lang.FALSE.lower()]
        or usrStr[0] == lang.NO[0].lower()
    ):
        return 0
    ...
    # on
    if (
        usrStr in ["1", lang.ON.lower(), lang.YES.lower(), lang.TRUE.lower()]
        or usrStr[0] == lang.YES[0].lower()
    ):
        return 1
    ...
    # cancel
    if (
        usrStr in ["-1", lang.CANCEL.lower(), lang.BREAK.lower()]
        or usrStr[0] == lang.CANCEL[0].lower()
    ):
        return -1
...
def sigDigMeasure(num):
    ...
    if floatPart == 0:
        sigDig = 0
    elif floatPart < 0.0000001:
        sigDig = 8
    elif floatPart < 0.000001:
        sigDig = 7
    elif floatPart < 0.00001:
        sigDig = 6
    elif floatPart < 0.0001:
        sigDig = 5
    elif floatPart < 0.001:
        sigDig = 4
    elif floatPart < 0.01:
        sigDig = 3
...
def _softWrap(msg, charW, indentW):
    ...
    if indentW > 0:
        ...
        kwargs = {"replace_whitespace": 0, "break_long_words": 1}
...
def _bundleLines(charW, indentW, bundledLines):
    ...
    for lineCount in range(0, lineNo):
        for bundleCount in range(0, bundleNo):
            ...
            if (
                bundleCount >= 1
                and bundleCount == (bundleNo - 1)
                and lineCount != (lineNo - 1)
            ):
                if indentW == 0:
...
def formatVariCol(
    ...
    for line in entryLines:
        ...
        for entry in line:
            ...
            if (
                bufList[colCount] != 0
                and fitColWidths[colCount] >= minWidthList[colCount]
            ):
...
def graphNumber(min, max, value, charWidth=50, emptyChar=".", fullChar="+"):
    ...
    for counter in range(0, charWidth):
        if pcentValue >= pcentPosition and pcentValue < (pcentPosition + pcentInc):
            str.append(fullChar)
...
def graphLabeledRuler(
    llabel, rlabel, charWidth=50, divisions=5, emptyChar=".", fullChar="+"
):
    ...
    rulerStr = "%s%s%s" % (
        rulerStr[:startLeftReplace],
        llabel,
        rulerStr[endLeftReplace:],
    )
    rulerStr = "%s%s%s" % (
        rulerStr[:startRightReplace],
        rlabel,
