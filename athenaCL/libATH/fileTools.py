...
class GatherSrc(DirWalk):
    ...
    def constrainNames(self, fileName):
        """constrains files to a particular set"""
        if fileName[-3:] == ".py" or fileName[-4:] == ".xml" or fileName[-4:] == ".cgi":
            return fileName
        else:
            return None
...
class FileStats(DirWalk):
    ...
    def reportCount(self):
        ...
        for entry in self.dirList:
            print(entry)
        print("\n%s lines of code found for %s .py files" % (sum, len(self.fileList)))
...
class ConvertBreaks:
    ...
    def batchConvert(self):
        selNames, selPaths = osTools.dirGather(self.searchDir, ["txt", "py", "xml"])
...
class BundleFiles:
    ...
    def __init__(self, searchDir=None):
        ...
        selNames, selPaths = osTools.dirGather(
            self.searchDir, self.athDataEXT
        )  # gather .xml, .sco, .orc
        selPaths.sort()
        bundleDict = {}
...
class ProofSheet:
    ...
    def htmlPrep(self):
        ...
        self.wpObj = htmlTools.WebPage(
            self.totalWIDTH, self.title, self.colorDict, self.fontDict
        )
...
class ImageSheet(ProofSheet):
    ...
    def writePage(self):
        ...
        for target, dst in self.procFiles:
            ...
            if fileObj.relDir == None:
                relDirStr = "/"
            else:
                relDirStr = fileObj.relDir
...
class AudioSheet(ProofSheet):
    ...
    def writeMediaPage(self, count, label, mediaSrc, width):
        ...
        self.mpObj = htmlTools.WebPage(width, label, self.colorDict, self.fontDict)
        ...
        colList = []
        colList.append(
            {"width": width, "align": "center", "color": None, "content": caption}
        )
        body.append(self.mpObj.nTable(colList))
    ...
    def writePage(self):
        ...
        for target in self.allPaths:
            ...
            linkStr = self.wpObj.pText(
                popObj.linkToPop(fileName, name, mediaWidth, mediaHeight, 0)
                + linkStrAlt
            )
            dirStr = self.wpObj.pText("%s" % (relDirStr))
            if fileObj.audioType == None:
                dataStr = self.wpObj.pText(
                    "%s MB | %s" % (fileObj.sizeMb, fileObj.mTimeStr)
                )
            else:
                dataStr = self.wpObj.pText(
                    "%s MB | %s | %s | %s"
...
class CompareDir:
    ...
    def report(self):
        ...
        print(
            "total files\t\t%s\t%s\t%s"
            % (
                len(list(self.fileDictA.keys())),
                len(list(self.fileDictB.keys())),
                len(list(self.mergedFileDict.keys())),
            )
        )
        print(
            "missing dirs\t\t%s\t%s" % (len(self.dirMissListA), len(self.dirMissListB))
        )
        print(
            "missing files\t\t%s\t%s"
            % (len(self.fileMissListA), len(self.fileMissListB))
        )
        print(
            "outdated files\t\t%s\t%s"
            % (len(self.fileOutdateListA), len(self.fileOutdateListB))
        )
...
class Profiler:
    ...
    def runProfile(
        self, methodStr=None, statFile=None, recalibrate=1, givenCalibrationNum=None
    ):
