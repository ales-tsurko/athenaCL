...
def selectEventModeOrc(emName):
    ...
    'generalMidi'
    >>> selectEventModeOrc('superColliderNative')
    'superColliderNative'
    """
    ...
    elif emName == "superColliderNative":
...
def selectOutEngineOrc(emName, oeName):
    ...
    if oeName not in outputEngineNames:
        raise Exception("bad Output name: %s" % oeName)
...
class EventSequence:
    ...
    def interpolate(
        ...
        if self.__len__() != len(eventFrameIndex):
            raise ValueError(
                "tFrameArray does not contain all events stored: %s, %s"
                % (self.__len__(), len(eventFrameIndex))
            )
        ...
        if type in ["off", None]:
            ...
            for i in iForward:
                ...
                if i != iForward[-1]:  # if not the last
                    ...
                    altDur = abs(
                        self._eventList[j]["time"] - self._eventList[j - 1]["time"]
                    )
                else:  # last index
...
class EventSequenceSplit:
    ...
    def __init__(self, srcObj, srcFmt="t", eventCount=None, refresh=0, aoInfo=None):
        ...
        environment.printDebug(
            ["EventSequenceSplit got srcObj, srcFmt", srcObj, srcFmt]
        )
...
class _OutputEngine:
    def __init__(self, emObj, fpRef, ao):
        ...
        emObj is the  currently active EventMode
    ...
    def _orcSelect(self):
        ...
        in these cases, the engine's orc is specified by the emObj
        """
        # base eventMode name, as well as engine's name
        orcName = selectOutEngineOrc(self.emObj.name, self.name)
            ...
            if tOrcObj.name == self.orcObj.name:  # assume match, full compat
                ...
                if tOrcObj.name in self.orcIncompat:
                    environment.printWarn(
                        [
                            lang.WARN,
                            "texture %s (instrument %s, orchestra %s) incompatible with OutputEngine orchestra %s; change EventMode or edit instrument."
                            % (t, inst, tOrcObj.name, self.orcObj.name),
                        ]
                    )
    ...
    def _fmtHeadSco(self, headStr="", prepend=""):
        ...
        scoHead.append("%s %s\n" % (prepend, timeStr))
        scoHead.append(
            "%s output generator: %s (orchestra: %s)\n\n"
            % (prepend, self.name, self.orcObj.name)
        )
        if headStr != "":
    ...
    def _strValue(self, value, ljust=8, sigDig=6, prefix="", postfix=" "):
        ...
        if drawer.isStr(value):
            ...
            if sigDig == 0:
                val = int(val)  # convert
            return "%s%s%s" % (prefix, str(val).ljust(ljust), postfix)
    ...
    def _translateCsoundSilenceStr(self, orcMapMode, esObj):
        ...
        for event in el:
            ...
            msg.append(
                "".join(
                    [i, start, sus, midiNote, amp, phase, panX, panY, panZ, mason, cmt]
                )
            )
        msg.append("\n")
        return "".join(msg), label  # returns string
...
class EngineAudioFile(_OutputEngine):
    ...
    def _translatePoly(self):
        ...
        try:  # aiff tools may not be available on a given platform
            ...
            self.aObj = audioTools.AudioFile(self._outFormatToFilePath("audioFile"), 1)
        ...
        for tName in self.compatNames:
            ...
            if not tDict["mute"]:  # pass object to mix data into
                self.aObj = self._translateAudioFile(
                    orcMapMode, esObj, self.aObj, self.unitSynthesizerMethod
                )
    ...
    def _translatePoly(self):
        ...
        for tName in self.compatNames:
            ...
            if not tDict["mute"]:
                ...
                esObj = tDict["esObj"]
                data, label = self._translateSuperColliderNativeStr(orcMapMode, esObj)
                msg.append(label)
                msg.append(data)
...
"""
    ...
    def _writePost(self):
        ...
        self.fpRef["fpOutputView"] = self._outFormatToFilePath("superColliderTask")
...
class EngineCsoundNative(_OutputEngine):
    ...
    def _genCommandStr(self, csd=1):
        ...
        if os.name == "posix":
            ...
            batFileHead = "@ECHO off\nc:\ncd %s\n%s " % (
                self.fpRef["fpCsoundBinaryDir"],
                self.fpRef["fnCsoundBinary"],
            )

        renderOpt = (
            "-m2 "
            + "-d "  # msg level. Sum 1=amps, 2=out-of-range, 4=warnings
            + self.fpRef["audioFlag"]  # suppress all displays (-d) (ascii displays -g)
            + "-b1024 "
            + "-B1024 "  # sample frames (or -kprds) per sound I/O buffer, was 8192
            + audioDirFlag  # samples per hardware sound I/O buffer, was 8192
            + audioPathFlag  # Sound File Directory (mac only)
        )  # other plats use full path here
        ...
        if csd:
            ...
            pathOpt = (
                '"'
                + self._outFormatToFilePath("csoundOrchestra")
                + '" '
                + '"'
    ...
    def _writeCsd(self):
        ...
        if self.orcObj.srcStr == None:
            raise Exception("orchestra object has no src str")
        ...
        msg.append(optStr)
        msg.append(
            "%s\n%s%s\n\n" % ("<CsInstruments>", self.orcObj.srcStr, "</CsInstruments>")
        )
        msg.append("%s\n%s%s\n\n" % ("<CsScore>", self.polySeqStr, "</CsScore>"))
        msg.append("</CsoundSynthesizer>\n")
...
class EnginePureDataArray(_OutputEngine):
    ...
    def _getArrayDef(self, name, dataList, vCount):
        ...
        msg.append(
            "#X coords 0 %s %s %s %s %s 1;"
            % (dataMax, dataSize, dataMin, dataSize * 2, vPresentation)
        )
        ...
        for tName in self.compatNames:
            ...
            if not tDict["mute"]:
                ...
                for pmtrName in pmtrList:
                    ...
                    msg.append(
                        self._getArrayDef("%s-%s" % (tName, pmtrName), dataList, vShift)
                    )
                    vShift += 1
            ...
            for cName in list(tDict["TC"].keys()):
                if not tDict["TC"][cName]["mute"]:
                    ...
                    for pmtrName in pmtrList:
                        ...
                        msg.append(
                            self._getArrayDef(
                                "%s-%s-%s" % (tName, cName, pmtrName), dataList, vShift
                            )
                        )
                        vShift += 1
        self.msg = msg
...
class EngineMidiFile(_OutputEngine):
    ...
    def _write(self):
        ...
        midiObj = midiTools.MidiScore(
            self.trackList, self.fpRef["fnOutputPrime"], self.fpRef["optionMidiTempo"]
        )
        ...
        for tName in self.compatNames:
            ...
            for cName in list(tDict["TC"].keys()):
                if not tDict["TC"][cName]["mute"]:
                    ...
                    esObj = tDict["TC"][cName]["esObj"]
                    data, label = self._translateTextDelimitStr(
                        orcMapMode, esObj, delimit
                    )
                    msg.append(label)
                    msg.append(data)
        self.polySeqStr = "".join(msg)
...
class EventMode(object):
    ...
    def _getAudioFormatExt(self):
        ...
        >>> ao = athenaObj.AthenaObject()
        >>> a = EventMode(ao, 'cn')
        >>> post = a._getAudioFormatExt() # based on a user preference
        """
        # leave space after flag
        usrFileFormat = self.ao.external.getPref("external", "audioFormat")
        if usrFileFormat == "aif":
            return ".aif", "-A "
        elif usrFileFormat == "wav":
            return ".wav", "-W "
        elif usrFileFormat == "sd2":
            return ".sd2", ""
    ...
    def _initRefNames(self):
        ...
        all paths derived from fpOutputPrime as a root file.
        note: a future version might take fpOutputPrime as a directory and bundle
        content

        >>> from athenaCL.libATH import athenaObj
        >>> ao = athenaObj.AthenaObject()
        >>> a = EventMode(ao, 'cn')
        >>> a.setRootPath() # can be called to get temporary file
        >>> a._initRefNames()
        """
        if self.fpOutputPrime == None:
    ...
    def _buildPathPrime(self, ext):
        ...
        path = os.path.join(
            self.fpRef["fpOutputDirPrime"], (self.fpRef["fnOutputPrime"] + ext)
        )
        return path
    ...
    def _initRefPaths(self):
        ...
        self.fpRef["fpOutputDirPrime"], fileName = os.path.split(self.fpOutputPrime)
        ...
        self.fpRef["fpCsoundBinary"] = self.ao.external.getPref(
            "external", "csoundPath"
        )
    ...
    def _updateAthenaObjectInfo(self, fpRef):
        ...
        >>> ao = athenaObj.AthenaObject()
        >>> a = EventMode(ao, 'cn')
        >>> engineLib = a._engineAllocate('scn')
        >>> engineName = engineLib.keys()[0]
    ...
    def setRootPath(self, fpOutputPrime=None):
        """
        This sets the the path (generally an xml file) that is used to determine
        the name of all other files and the directory in which to write them.s
    ...
    def getWritePaths(self, fpOutputPrime):
    ...
    def process(self, input=None, usrOutRequest=[], refresh=1):
        ...
        for engineName in list(engineLib.keys()):
            try:  # will write orchestra if necessary
                ...
                self.outComplete = self.outComplete + engineLib[engineName].outComplete
...
def factory(eventModeName, ao):
    eventModeName = eventModeParser(eventModeName)
    assert eventModeName != None
    return EventMode(ao, eventModeName)
...
class Test(unittest.TestCase):
    ...
    def testEventList(self):
        ...
        textureParameters["panQ"] = ("constant", 0.5)
