...
class Clone:
    ...
    def load(self, pmtrQDict, auxNo, auxFmt, mute=0):
        ...
        self.pmtrObjDict["time"] = parameter.factory(
            self.pmtrQDict["time"], "filterPmtrObjs"
        )
        self.pmtrObjDict["sus"] = parameter.factory(
            self.pmtrQDict["sus"], "filterPmtrObjs"
        )
        self.pmtrObjDict["acc"] = parameter.factory(
            self.pmtrQDict["acc"], "filterPmtrObjs"
        )
        self.pmtrObjDict["fieldQ"] = parameter.factory(
            self.pmtrQDict["fieldQ"], "filterPmtrObjs"
        )
        self.pmtrObjDict["octQ"] = parameter.factory(
            self.pmtrQDict["octQ"], "filterPmtrObjs"
        )
        self.pmtrObjDict["ampQ"] = parameter.factory(
            self.pmtrQDict["ampQ"], "filterPmtrObjs"
        )
        self.pmtrObjDict["panQ"] = parameter.factory(
            self.pmtrQDict["panQ"], "filterPmtrObjs"
        )
    ...
    def _editRestore(self, attrName, pmtrName, data, refDict):
        ...
        if not ok:
            raise ValueError("clone edit: original data cannot be restored: %s" % msg)
        ...
        if cName == None:  # delete all of a texture's clones
            if tName in list(self._tRef.keys()):
                ...
                environLocal.printDebug[
                    "attempting to remove clone from texture %s not stored in tRef"
                    % tName
                ]
        else:
            if tName in list(self._tRef.keys()):
