...
class TEmapCanvas:
    def __init__(
        self, ao=None, tiMapDict=None, barHEIGHT=8, winWIDTH=700, fmt="tk", master=None
    ):
        ...
        self.c = imageTools.Canvas(
            fmt, self.winWIDTH, self.winHEIGHT, COLORbgAbs, "TEmap", master
        )
        # draw margin rectangles
        self.c.rectangle(0, 0, self.winWIDTH, self.tMarginSize, COLORbgMargin, None, 0)
        self.c.rectangle(0, 0, self.lMarginSize, self.winHEIGHT, COLORbgMargin, None, 0)
        self.c.rectangle(
            0,
            self.tMarginSize + self.mapHEIGHT,
            self.winWIDTH,
            self.winHEIGHT,
            COLORbgMargin,
            None,
        ...
        self.c.rectangle(
            self.mapWIDTH + self.lMarginSize,
            0,
            self.winWIDTH,
            self.winHEIGHT,
            COLORbgMargin,
        ...
        for keyPartition in range(0, noPartitions):
            ...
            if keyPartition == 0:  # first needs to be bkg color
                ...
                self.c.line(
                    xStart, yStart + self.tMarginSize, xStart, yEnd, COLORbgMargin, 0.5
                )
                timeStr = "%.1f" % ((self.maxTime / noPartitions) * keyPartition)
                # this adds numbers to key; buffer by a gutter from edge of window
                self.c.gridText(
                    xCurrentPos, self.mapGUTTER, "nc", timeStr, fontTitle, COLORtxUnit
                )
            else:  # use sub pixel line size for vector version
                self.c.line(
                    xStart, yStart + self.tMarginSize, xStart, yEnd, COLORbgGrid, 0.5
                )
        ...
        for tName in tNameList:
            ...
            barOffsetStart = int(round(self.mapWIDTH * (float(tStart) / self.maxTime)))
            # must add 1 here to get off margin, and into map
            xStart = self.lMarginSize + barOffsetStart + 1

            barOffsetEnd = int(round(self.mapWIDTH * (float(tEnd) / self.maxTime)))
            ...
            # create head box
            self.c.rectangle(
                xStart, yHeadStart, xEnd, yHeadEnd, COLORfgMainFrame, None, 0
            )
            if self.tiMapDict[tName]["muteStatus"] == 0:  # normal
                self.c.rectangle(
                    xStart, yBodyStart, xEnd, yBodyEnd, COLORfgMain, None, 0
                )
            else:  # silenced
                self.c.rectangle(
                    xStart, yBodyStart, xEnd, yBodyEnd, None, COLORfgMain, 1
                )
            # create texture label
            self.c.gridText(
                self.mapGUTTER, yBodyStart, "nw", tName, fontTitle, COLORtxTitle
            )
            # shift down
            ...
            for cName in cNameList:
                ...
                barOffsetEnd = int(round(self.mapWIDTH * (float(cEnd) / self.maxTime)))
                ...
                # create head box
                self.c.rectangle(
                    xStart, yHeadStart, xEnd, yHeadEnd, COLORfgAltFrame, None, 0
                )

                if self.tiMapDict[tName]["cloneDict"][cName]["muteStatus"] == 0:
                    self.c.rectangle(
                        xStart, yBodyStart, xEnd, yBodyEnd, COLORfgAlt, None, 0
                    )
                else:  # silenced
                    self.c.rectangle(
                        xStart, yBodyStart, xEnd, yBodyEnd, None, COLORfgAlt, 1
                    )
