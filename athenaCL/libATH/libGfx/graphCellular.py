...
class CAmapCanvas:
    def __init__(self, ao=None, caData=[], caDataDstValues=None, 
        ...
        self.c.rectangle(0, 0, self.lMargin-1, self.winHEIGHT, 
                              self.marginColor, None, 0)         
        # bottom
        self.c.rectangle(0, self.winHEIGHT-self.bMargin, 
                              self.winWIDTH, self.winHEIGHT, self.marginColor, None, 0)       
        # right
        self.c.rectangle(self.winWIDTH-self.rMargin, 0, 
                              self.winWIDTH, self.winHEIGHT, self.marginColor, None, 0)       
        ...
        if title != None:
            ...
            for chunk in title.split('\n'):
                self.c.gridText(self.tMargin, y, 'nw', chunk, 
                                'micro', COLORtxTitle, 1, None, 1, 1)
                y = y + 10
    ...
    def _makeBitMap(self):
        ...
        for row in self.caData:
            ...
            for col in row:
                ...
                color = imageTools.RGBToHTMLColor(
                                imageTools.FloatToRGB(colFloat))
...
def display(usrStr, rule=0, mutation=0):
    ...
    obj = CAmapCanvas(None, ca.getCells('table', 0, ca.spec.get('s'), None, 
                            ca.spec.get('c'), ca.spec.get('w'),), 
                            ca.dstValues, 2, 2, 'png', None, ca.repr('brief'))
    obj.show()
