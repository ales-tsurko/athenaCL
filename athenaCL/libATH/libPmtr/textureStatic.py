...
class OrnamentLibrarySelect(basePmtr.StaticParameterTexture):
    def __init__(self, args, refDict):
        ...
        self.argTypes = ["str"]
        self.argNames = [
            "libraryName: chromaticGroupC, diatonicGroupA, diatonicGroupB, microGroupA, microGroupB, microGroupC, trillGroupA, off"
        ]
    ...
    def checkArgs(self):
        if self.switch("libraryName") not in self.refDict["ornGroupNames"]:
            ornNames = drawer.listScrub(self.refDict["ornGroupNames"], None, "noQuote")
...
class LoopWithinSet(_SwitchOnOff):
    def __init__(self, args, refDict):
        ...
        self.argDefaults = [
            "on",
        ]  # should be on, as may be a single note path
...
class LevelOctaveMonophonic(_SwitchLevelMonophonic):
    def __init__(self, args, refDict):
        ...
        self.argDefaults = [
            "event",
        ]  # default for octave is usually event
        _SwitchLevelMonophonic.__init__(self, args, refDict)  # call base init
...
class InterpolationMethodControl(basePmtr.StaticParameterTexture):
    def __init__(self, args, refDict):
        ...
        self.argNames = [
            "method: linear, halfCosine, power",
        ]
        self.argDefaults = [
            "linear",
        ]
