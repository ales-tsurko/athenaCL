...
class CyclicGen(basePmtr.Parameter):
    ...
    def __init__(self, args, refDict):
        ...
        self.argDefaults = ['ud', 0, 1, .125]    
        self.argDemos = [['d',0,1,.125],
                             ]
        # check raw arguments for number, type
        ok, msg = self._checkRawArgs()
        if ok == 0: raise error.ParameterObjectSyntaxError(msg) # report error
    ...
    def repr(self, format=''):
        ...
        return '%s, %s, %s, %s, %s' % (self.type, 
            self.directionSrc, typeset.anyDataToStr(self.min), 
            typeset.anyDataToStr(self.max), typeset.anyDataToStr(self.inc))
...
class BasketGen(basePmtr.Parameter):
    def __init__(self, args, refDict):
        ...
        self.argDefaults = ['rc', [0,.25,.25,1]]
        self.argDemos = [['oo',[0,.1,.2,.4,.8,.6,.5,1]],
                              ['rw',[0,.1,.2,.4,.8,.6,.5,1]],
                             ]
        # check raw arguments for number, type
        ok, msg = self._checkRawArgs()
    ...
    def repr(self, format=''):
        return '%s, %s, %s' % (self.type, self.control, 
                                      self._scrubList(self.list))
...
class SieveList(basePmtr.Parameter):
    def __init__(self, args, refDict):
        ...
        self.argNames = ['logicalString', 'zMin', 'zMax', 
    ...
    def repr(self, format=''):
        ...
        msg.append('%s, %s, %s, %s, %s, %s' % (self.type, str(self.sieveObj), 
                    typeset.anyDataToStr(self.zMin), typeset.anyDataToStr(self.zMax),
                    self.format, self.control))
        if format == 'argsOnly': 
            return ''.join(msg)
...
class ValueSieve(basePmtr.Parameter):
    def __init__(self, args, refDict):
        ...
        self.argNames = ['logicalString', 'length', 'min', 'max', 
                              'selectionString']
        self.argDefaults = ['3&19|4&13@11',360,0,1,'oo']
        self.argDemos = [['3&19|4&13@11|5@2&15@2',120,0,1,'rw'],
                             ['3&19|4&13@11',240,
                              ('bpp','e','s',((0,0),(80,48),(120,30)),-1.25),
                              ('bpp','e','s',((0,100),(80,52),(120,100)),1.25),'oc'],
                             ['3&19|4&13@11',120,
                              ('bpp','e','s',((0,0),(80,48),(120,30)),-1.25),
                              ('bpp','e','s',((0,100),(80,52),(120,100)),1.25),'rp'],
                              ]
        # check raw arguments for number, type
        ok, msg = self._checkRawArgs()
        if ok == 0: raise error.ParameterObjectSyntaxError(msg) # report error
    ...
    def repr(self, format=''):
        ...
        msg = []
        msg.append('%s, %s, %s, (%s), (%s), %s' % (self.type, str(self.sieveObj), 
                    typeset.anyDataToStr(self.length), minStr, maxStr, self.control))
        if format == 'argsOnly': 
            return ''.join(msg)
        # show sieve if not argsOnly
        msg.append('\n')
...
class SieveFunnel(basePmtr.Parameter):
    ...
    def repr(self, format=''):
        ...
        msg = []
        msg.append('%s, %s, %s, (%s), (%s), (%s)' % (self.type, 
                    str(self.sieveObj), 
                    typeset.anyDataToStr(self.length), minStr, maxStr, pmtrStr))
        if format == 'argsOnly': 
            return ''.join(msg)
        # show sieve if not argsOnly
        msg.append('\n')
    ...
    def __call__(self, t=None, refDict=None):
        ...
        self.currentValue = unit.denorm(unitVal, 
                                  self.minObj(t, refDict),   self.maxObj(t, refDict))
        return self.currentValue
...
class ListPrime(basePmtr.Parameter):
    def __init__(self, args, refDict):
        ...
        self.argNames = ['start', 'length', 
                              'format: integer, width, unit, binary', 
                                'selectionString']
        self.argDefaults = [2, 50, 'int', 'oc']
        self.argDemos = [[-100,100,'wid','rc'],
                              [200,-30,'bin','rp'],
                              ]
    ...
    def repr(self, format=''):
        msg = []
        msg.append('%s, %s, %s, %s, %s' % (self.type, 
                    typeset.anyDataToStr(self.start), typeset.anyDataToStr(self.length),
                    self.format, self.control))
        if format == 'argsOnly': 
            return ''.join(msg)
        # show sieve if not argsOnly
...
class ValuePrime(basePmtr.Parameter):
    def __init__(self, args, refDict):
        ...
        self.argDemos = [[100,20,['bphc','e','l',[(0,.5),(120,1)]],1,'rp'],
                              ]
        # check raw arguments for number, type
        ok, msg = self._checkRawArgs()
        if ok == 0: raise error.ParameterObjectSyntaxError(msg) # report error
...
class CaList(basePmtr.Parameter):
    def __init__(self, args, refDict):
        ...
        self.argNames = ['caSpec', 'parameterObject: rule', 
                              'parameterObject: mutation', 
                              'tableExtractionString', # defined in base class
                              'selectionString']
        self.argDefaults = ['f{f}i{c}x{81}y{120}',.25,0.0005,'sc','oc']
        self.argDemos = [
        ...
        if drawer.isNum(mutation): mutation = ('c', mutation)
        ...
        for i in range(1, self.ca.spec.get('yTotal')): # already got zero
            self.ca.gen(1, self.rule(i, self._refDictSim), 
                                self.mutation(i, self._refDictSim))
        norm = 0 # do not normalize
        self.segment = self.ca.getCells(self.tableFormat, norm, 
    ...
    def repr(self, format=''):
        ...
        msg = []
        msg.append('%s, %s, (%s), (%s), %s, %s' % (self.type, str(self.ca), 
                    rule, mutation, self.tableFormat, self.control))
        if format == 'argsOnly': 
            return ''.join(msg)
        return ''.join(msg)
...
class CaValue(basePmtr.Parameter):
    def __init__(self, args, refDict):
        ...
        self.argTypes = ['str', ['num', 'list'], ['num', 'list'], 'str', 
                             ['num','list'], ['num','list'], 'str']
        self.argNames = ['caSpec', 'parameterObject: rule', 
                              'parameterObject: mutation', 
                              'tableExtractionString', # options defined in base class
                              'min', 'max', 'selectionString']
        self.argDefaults = ['f{s}', ('c',110), ('c',0), 'sr', 0, 1, 'oc']
        self.argDemos = [ ['f{s}x{81}y{120}k{2}r{1}i{r}',('bpl','e','s',((0,30),(119,34))),0.05,'sr',0,1,'oc'], ['f{t}x{81}y{120}k{3}r{1}w{12}',1842,('bpl','e','l',((0,0),(80,.02))),'sr',('ws','e',15,0,0,.4),1,'oc'],                                
                             ]
        # check raw arguments for number, type
        ok, msg = self._checkRawArgs()
        if ok == 0: raise error.ParameterObjectSyntaxError(msg) # report error
    ...
    def repr(self, format=''):
        ...
        maxStr = self.maxObj.repr(format)
        msg = []
        msg.append('%s, %s, (%s), (%s), %s, (%s), (%s), %s' % (self.type,
            str(self.ca), rule, mutation, self.tableFormat, minStr, maxStr, 
            self.control))
        if format == 'argsOnly': 
            return ''.join(msg)
        return ''.join(msg)
    ...
    def __call__(self, t=None, refDict=None):
        self.currentValue = unit.denorm(self.selector(), 
                                  self.minObj(t, refDict), self.maxObj(t, refDict))
        return self.currentValue
...
class LogisticMap(basePmtr.Parameter):
    def __init__(self, args, refDict):
        ...
        self.argTypes = ['num', ['str','list','num'], 
                                        ['num','list'], ['num','list']]
        self.argNames = ['initValue', 'parameterObject: p value', 'min', 'max']
        self.argDefaults = [.5,('wt','e',90,0,2.75,4),0,1]
        self.argDemos = [[0.1, ('bg', 'rw', (3,3,3,3.2,3.2,3.2,3.9,3.9,3.9)), 
                                0, 1],
                              [.5,('ig',('bg','rc',(3,3.2,3.5699461)),
                                            ('bg','rc',(5,7,9))),
                                    ('bpl','e','l',((0,.5),(60,0),(120,.5))),
                                    ('bpl','e','l',((0,.5),(40,3)))],
                             ]
        # check raw arguments for number, type
        ok, msg = self._checkRawArgs()
        ...
        ref = {
            3.2      : ['bi', '2'],
            3.44951  : ['quad', '4'],
            3.5699461 : ['chaos', ], # this value does not seem to be cahaotic
            3.57         : ['periodic01',],
                }
    ...
    def repr(self, format=''):
        ...
        maxStr = self.maxObj.repr(format)
        return '%s, %s, (%s), (%s), (%s)' % (self.type, 
                 typeset.anyDataToStr(self.xInit), pmtrStr, minStr, maxStr)
    ...
    def __call__(self, t=None, refDict=None):
        ...
        self.currentValue = unit.denorm(self.x, 
                                  self.minObj(t, refDict),   self.maxObj(t, refDict))
        return self.currentValue
...
class HenonBasket(basePmtr.Parameter):
    def __init__(self, args, refDict):
        ...
        self.argTypes = ['num', 'num', 
                             ['num','list'], ['num','list'],
                              'int', 'str',
                             ['num','list'], ['num','list'], 'str']
        self.argNames = ['xInit', 'yInit',
                              'parameterObject: a value',
                              'parameterObject: b value',
                              'valueCount', 'valueSelect: x, y, xy, yx', 
                              'min', 'max', 'selectionString']
                              
        self.argDefaults = [.5, .5, 1.4, 0.3, 1000, 'x', 0, 1, 'oc']
        self.argDemos = [[.5,.5,.5,.8,1000,'yx'],
                              [.5,.5,('cg','ud',0,.9,.05),.3,1000,'xy'],
                             ]
        # check raw arguments for number, type
        ok, msg = self._checkRawArgs()
        if ok == 0: raise error.ParameterObjectSyntaxError(msg) # report error
        ...
        # do processing:
        self.basket = []
        self.henon = chaos.Henon(self.a(0, self._refDictSim), 
        ...
        while i <= self.valueCount: # start at 0
            if i == 0: # dont refresh a and b
                ...
                x, y = self.henon(self.a(i, self._refDictSim), 
                                        self.b(i, self._refDictSim))
            for char in self.valueSelect:
                if char == 'x': self.basket.append(x)
                elif char == 'y': self.basket.append(y)
                i = i + 1
    ...
    def repr(self, format=''):
        ...
        minStr = self.minObj.repr(format)
        maxStr = self.maxObj.repr(format)
        return '%s, %s, %s, (%s), (%s), %s, %s, (%s), (%s), %s' % (self.type, 
                        self.xInit, self.yInit, aStr, bStr,
                        typeset.anyDataToStr(self.valueCount), self.valueSelect, 
                        minStr, maxStr, self.control)
...
class LorenzBasket(basePmtr.Parameter):
    def __init__(self, args, refDict):
        ...
        # a, b, xinit, yinit, min, max, select
        self.argTypes = ['num', 'num', 'num',
                             ['num','list'], ['num','list'], ['num','list'],
                              'int', 'str',
                             ['num','list'], ['num','list'], 'str']
        self.argNames = ['xInit', 'yInit', 'zInit',
      ...
      'valueSelect: x,y,z,xy,xz,yx,yz,zx,zy,xyz,xzy,yxz,yzx,zxy,zyx', 
        ...
        self.argDefaults = [1.0, 1.0, 1.0, 28, 10, 2.66666, 
                                  1000, 'xyz', 0, 1, 'oc']
        self.argDemos = [
                      [.5,1.5,10,('cg','d',1,80,1.5),10,12.4,1000,'x',0,1,'oc'],
                             ]
        # check raw arguments for number, type
        ok, msg = self._checkRawArgs()
        if ok == 0: raise error.ParameterObjectSyntaxError(msg) # report error
        ...
        # do processing:
        self.basket = []
        self.lorenz = chaos.Lorenz(self.r(0, self._refDictSim), 
                                            self.s(0, self._refDictSim),
                                            self.b(0, self._refDictSim),
                                            self.xInit, self.yInit, self.zInit)
        i = 0
        tSim = 0 # simulated time
        while i <= self.valueCount: # start at 0
            if i == 0: # dont refresh values
                ...
                x, y, z = self.lorenz(self.r(tSim, self._refDictSim), 
                                             self.s(tSim, self._refDictSim),
                                             self.b(tSim, self._refDictSim))
            for char in self.valueSelect:
                if char == 'x': self.basket.append(x)
                elif char == 'y': self.basket.append(y)
                elif char == 'z': self.basket.append(z)
                i = i + 1
            tSim = tSim + 1 # increment for each 
        self.normSeries = unit.unitNormRange(self.basket)
        self.selector = basePmtr.Selector(self.normSeries, self.control)
    ...
    def repr(self, format=''):
        ...
        return '%s, %s, %s, %s, (%s), (%s), (%s), %s, %s, (%s), (%s), %s' % (
                        self.type, self.xInit, self.yInit, self.zInit, 
                        rStr, sStr, bStr,
                        typeset.anyDataToStr(self.valueCount), self.valueSelect, 
                        minStr, maxStr, self.control)
...
class Noise(basePmtr.Parameter):
    ...
    def repr(self, format=''):
        ...
        maxStr = self.maxObj.repr(format)
        return '%s, %s, (%s), (%s), (%s)' % (self.type, 
                  typeset.anyDataToStr(self.resolution), pmtrStr, minStr, maxStr)
    ...
    def __call__(self, t=None, refDict=None):
        ...
        self.currentValue = unit.denorm(self.game(), # calling game gets sum 
                                  self.minObj(t, refDict),   self.maxObj(t, refDict))
        return self.currentValue
...
class Accumulator(basePmtr.Parameter):
    def __init__(self, args, refDict):
        ...
        self.argDemos = [[0,('ws','e',20,0,-.5,1.5)], # wavy accumulator
                             ]
        # check raw arguments for number, type
        ok, msg = self._checkRawArgs()
        if ok == 0: raise error.ParameterObjectSyntaxError(msg) # report error
    ...
    def repr(self, format=''):
        pmtrStr = self.pmtrObj.repr(format)
        return '%s, %s, (%s)' % (self.type, typeset.anyDataToStr(self.init),
                                         pmtrStr)
...
class Mask(basePmtr.Parameter):
    def __init__(self, args, refDict):
        ...
        self.argNames = ['boundaryString: limit, wrap, reflect',
        ...
        self.argDefaults = ['l',('ws','e',60,0,.5,0),('wc','e',90,0,.5,1),
                                        ('ru',0,1)]
        self.argDemos = [ ['w',('bpl','e','l',((0,0),(90,.5))),
                                      ('bpl','e','l',((0,1),(90,.5))),
                                      ('ws','e',30,0,0,1)],     
                                ['r',('ws','e',60,.25,.7,1),
                                      ('bpl','e','l',((0,.4),(90,0),(120,.4))),
                                      ('ws','e',24,0,0,1)],
                                ]
        # check raw arguments for number, type
        ok, msg = self._checkRawArgs()
    ...
    def repr(self, format=''):
        ...
        return '%s, %s, (%s), (%s), (%s)' % (self.type, self.boundaryMethod, 
                                                         a, b, f)
...
class MaskScale(basePmtr.Parameter):
    def __init__(self, args, refDict):
        ...
        self.argNames = ['parameterObject: source Generator', 
                              'valueCount',
                              'min', 'max', 'selectionString']       
        self.argDefaults = [('lp',100,120,'w','oc'),120,
                                    ('bphc','e','l',((0,0),(120,-3))),3,'oc']
        self.argDemos = []
        # check raw arguments for number, type
        ok, msg = self._checkRawArgs()
        if ok == 0: raise error.ParameterObjectSyntaxError(msg) # report error
    ...
    def repr(self, format=''):
        ...
        msg = []
        msg.append('%s, (%s), %s, (%s), (%s), %s' % (self.type, 
            genStr, typeset.anyDataToStr(self.valueCount), 
            minStr, maxStr, self.control))
        if format == 'argsOnly': 
            return ''.join(msg)
        return ''.join(msg)
...
class FunnelBinary(basePmtr.Parameter):
    ...
    def repr(self, format=''):
        ...
        b = self.boundingPmtrObjB.repr(format)
        f = self.fillPmtrObj.repr(format)
        return '%s, %s, (%s), (%s), (%s), (%s)' % (self.type, 
            self.thresholdMatch, h, a, b, f)
...
class IterateGroup(basePmtr.Parameter):
    ...
    def __init__(self, args, refDict):
        ...
        self.argDemos = [[('wc','e',30,0,0,1),('wt','e',20,0,4,-1)],
                              ]
        # check raw arguments for number, type
        ok, msg = self._checkRawArgs()
        if ok == 0: raise error.ParameterObjectSyntaxError(msg) # report error
...
class IterateHold(basePmtr.Parameter):
    ...
    def __init__(self, args, refDict):
        ...
        self.argDefaults = [('ru',0,1),('bg','rc',[2,3,4]),
                                                 ('bg','oc',[12,24]),'oc']
        self.argDemos = [[('ws','e',30,0,0,1),('bg','rc',[3,4,5]),
                                              ('bg','oc',[6,12,18]),'oo'],
                             ]
...
class IterateCross(basePmtr.Parameter):
    def __init__(self, args, refDict):
        ...
        self.argNames = ['parameterObject: first source Generator', 
                              'parameterObject: second source Generator',
             'parameterObject: interpolation between first and second Generator',
                             ]        
        self.argDefaults = [('ws','e',30,0,0,1),('wp','e',30,0,0,1),
                                  ('bpl','e','l',((0,0),(120,1)))]
        self.argDemos = [[('ws','e',30,0,0,1),('ru',0,1),
                                  ('bpl','e','l',((0,0),(120,1)))],
                             ]
        # check raw arguments for number, type
        ok, msg = self._checkRawArgs()
    ...
    def repr(self, format=''):
        ...
        controlStr = self.controlObj.repr(format)             
        return '%s, (%s), (%s), (%s)' % (self.type, genStrOne, 
                                                    genStrTwo, controlStr)
...
class IterateSelect(basePmtr.Parameter):
    ...
    def __init__(self, args, refDict):
        ...
        self.argDefaults = [('ru',0,1),('bg','rc',[10,11,12]),
                                                 ('bg','oc',[12,24]),('rb',.15,.15,0,1)]
        self.argDemos = [[('lp',20,20,'int','oc'),('c',20),
                                    ('c',20),('rb',.2,.2,0,1)],
                             ]
...
# after below, reset trigger gate
    ...
    def __init__(self, args, refDict):
        ...
        self.argTypes = ['str', 'list', 'list', 'list']
        self.argNames = ['comparison',
                              'parameterObject: source Generator', 
                              'parameterObject: trigger Generator', 
                              'parameterObject: trigger threshold Generator',
                             ]
        
        self.argDefaults = ['gt',('ru',0,1),('wsd','e',10,0,0,1),('c', .5)]
        self.argDemos = [['e',('ru',0,1),('wp','e',20,0,0,1),('c',1)],
...
class Quantize(basePmtr.Parameter):
    ...
    def __init__(self, args, refDict):
        ...
        self.argNames = [
                        ...
                        'parameterObject: source Generator']
        self.argDefaults = [('c', 0), ('c', .25), 1, ('c',1), ('ru', 0, 1)]
        self.argDemos = [[('c',0),('bg','oc',[.05,.2]),2,
                                          ('bpl','e','l',((0,1),(120,.5))),
                                          ('wpu','e',20,-2,0,0,1)],
                                # expanding wave w/ diminishing attraction
                                [('ws','e',60,0,1.25,1.75),('cg','lud',.3,.9,0.006),1,
                                 ('bpl','e','l',((0,1),(40,1),(120,0.25))),('ru',0,1)], 
                                ]
        # check raw arguments for number, type
        ok, msg = self._checkRawArgs()
        if ok == 0: raise error.ParameterObjectSyntaxError(msg) # report error
    ...
    def repr(self, format=''):
        ...
        f = self.fillPmtrObj.repr(format)
        return '%s, (%s), (%s), %s, (%s), (%s)' % (self.type, g,
                     s, typeset.anyDataToStr(self.stepCount), p, f)
...
class MarkovValue(basePmtr.Parameter):
    def __init__(self, args, refDict):
        ...
        self.argDemos = [['a{0}b{.2}c{.4}d{.6}e{.8}f{1}:{a=3|b=6|c=8|d=8|e=5|f=2}a:{b=3}b:{a=2|c=4}c:{b=3|d=5}d:{a=1|c=4|e=3}e:{d=3|f=2}f:{e=2}a:b:{c=3}b:a:{b=2}b:c:{d=4}c:b:{a=2|c=1}c:d:{a=1|c=1|e=3}d:a:{b=1}d:c:{b=3|d=1}d:e:{d=1|f=2}e:d:{c=3}e:f:{e=2}f:e:{d=2}',('bpl','e','s',((0,0),(119,2)))],
                                ]
        # check raw arguments for number, type
        ok, msg = self._checkRawArgs()
        if ok == 0: raise error.ParameterObjectSyntaxError(msg) # report error
...
class MarkovGeneratorAnalysis(basePmtr.Parameter):
    ...
    def __init__(self, args, refDict):
        ...
        self.argDefaults = [('ws','e',30,0,0,1),30,2,
                                  ('mv','a{1}b{0}c{2}:{a=10|b=1|c=2}',('c',0))]
        self.argDemos = [[('bpp','e','l',((0,.5),(10,1),(15,0)),2),15,2,
                                ('bg','rw',(0,1,2,2,1))],
                              [('bg','oc',(.3,.3,.3,0,.9,.9,.6)),28,2,
                                ('mv','a{1}b{0}c{2}:{a=10|b=1|c=2}',('c',0))]
                             ]
        # check raw arguments for number, type
        ok, msg = self._checkRawArgs()
...
class FibonacciSeries(basePmtr.Parameter):
    def __init__(self, args, refDict):
        ...
        self.argDemos = [[40,20,0,1,'rc'],
                              [400,20,('ws','e',35,0,.5,0),
                                         ('cg','ud',.6,1,.025), 'oo'],
                             ]
        # check raw arguments for number, type
        ok, msg = self._checkRawArgs()
        ...
        self.minObj, self.maxObj = self._loadMinMax(self.args[2], self.args[3])
        # do setup processing
        self.series = chaos.fibonacciSeries(self.start, 
                                        (self.start + abs(self.length)))
        self.normSeries = unit.unitNormRange(self.series)
...
class _Operator(basePmtr.Parameter):
    def __init__(self, args, refDict):
        ...
        self.type = None
        self.argTypes = ['list', 'list']
        self.argNames = ['parameterObject: first value', 
...
class OneOver(basePmtr.Parameter):
    ...
    def __call__(self, t=None, refDict=None):
        ...
        try:
            self.currentValue = 1. / x # permit floats or ints
        except ZeroDivisionError:
            self.currentValue = 1
        return self.currentValue
