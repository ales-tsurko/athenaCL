...
class Ornament:
    ...
    def _ornLibrary(self):
        ...
        self.ornLib = {
            "trill.0,1.a": {
                ...
                "durInstPcentOffset": 0.034,
                "ampScalerMedian": 0.920,  # scale of base note
                "ampInstPcentOffset": 0.012,
            },
            "trill.0,1.b": {
                "contourForm": [0, 1],
                "ornStyle": "loop",
                ...
                "durInstPcentOffset": 0.042,
                "ampScalerMedian": 0.930,  # scale of base note
                "ampInstPcentOffset": 0.010,
            },
            "trillocto.0,12.a": {
                "contourForm": [0, 12],
                "ornStyle": "loop",
            ...
            "trillP5.0,7.a": {
                ...
                "durInstPcentOffset": 0.042,
                "ampScalerMedian": 0.930,  # scale of base note
                "ampInstPcentOffset": 0.015,
            },
            "turn.0,1,0,-1.a": {
                "contourForm": [0, 1, 0, -1],
                "ornStyle": "loop",
                "ornNotePcent": 0.47,
                "durOrnGoal": [0.90, 0.088, 0.96, 0.086],
                "durInstPcentOffset": 0.023,
                "ampScalerMedian": 0.940,  # scale of base note
                "ampInstPcentOffset": 0.025,
            },
            "turn.0,1,0,-1.b": {
                "contourForm": [0, 1, 0, -1],
                "ornStyle": "single",
                "ornNotePcent": 0.50,
                "durOrnGoal": [0.110, 0.088, 0.105, 0.091],
                "durInstPcentOffset": 0.023,
                "ampScalerMedian": 0.940,  # scale of base note
                "ampInstPcentOffset": 0.025,
            },
            ...
            "rise.-2,-1": {
                ...
                "durInstPcentOffset": 0.020,
                "ampScalerMedian": 0.940,  # scale of base note
                "ampInstPcentOffset": 0.012,
            },
            "rise.1,2": {
                "contourForm": [1, 2],  # "lift"
                "ornStyle": "scale",
                "ornNotePcent": 0.50,
                "durOrnGoal": [0.099, 0.105],
                "durInstPcentOffset": 0.032,
                "ampScalerMedian": 0.960,  # scale of base note
                "ampInstPcentOffset": 0.025,
            },
            "rise.-1": {
                "contourForm": [
            ...
            "fall.2,1": {
                ...
                "ornStyle": "single",
                "ornNotePcent": 0.50,
                "durOrnGoal": [0.099, 0.110],
                "durInstPcentOffset": 0.034,
                "ampScalerMedian": 0.950,  # scale of base note
                "ampInstPcentOffset": 0.008,
            },
            "fall.1,0,-1": {
                "contourForm": [1, 0, -1],
                "ornStyle": "single",
                "ornNotePcent": 0.50,
                "durOrnGoal": [0.092, 0.094, 0.099],
                "durInstPcentOffset": 0.025,
                "ampScalerMedian": 0.950,  # scale of base note
                "ampInstPcentOffset": 0.020,
            },
            "arc.-1,2,1": {
                "contourForm": [-1, 2, 1],
                "ornStyle": "scale",
                "ornNotePcent": 0.50,
                "durOrnGoal": [0.100, 0.105, 0.110],
                "durInstPcentOffset": 0.032,
                "ampScalerMedian": 0.940,  # .875 # scale of base note
                "ampInstPcentOffset": 0.023,
            },
            "arc.0,1": {
                "contourForm": [0, 1],
                "ornStyle": "single",
                "ornNotePcent": 0.50,
                "durOrnGoal": [0.099, 0.097],
                "durInstPcentOffset": 0.037,
                "ampScalerMedian": 0.950,  # scale of base note
                "ampInstPcentOffset": 0.018,
            },
            "fall.extended.a": {
                "contourForm": [
                    2,
                ...
                "ornStyle": "scale",
                "ornNotePcent": 0.70,
                "durOrnGoal": [0.088, 0.094, 0.086, 0.094, 0.090],
                "durInstPcentOffset": 0.022,
                "ampScalerMedian": 0.950,  # scale of base note
                "ampInstPcentOffset": 0.010,
            },
    ...
    # -----------------------------------------------------------------------||--
    def _getOrnLibrary(
        self, name=None, ornPos="release", pitchLang="path", microTone=0.5
    ):
    ...
    def psScale(self, pitchFormat, contourForm, psBase, microTone=0.5):
        ...
        if pitchFormat == "chromatic":
            for entry in contourForm:
                ...
                psReal = pitchTools.psToTempered(
                    pcSpace, octCurrent, self.temperamentObj, transCurrent
                )
            ...
            for entry in contourForm:  # treat scale step as microtone scaler
                ...
                if entry * microTone > entry * 2:
                    environment.printWarn(
                        [lang.WARN, "microtone large (%s)" % (entry * microTone)]
                    )
                trans = transCurrent + (entry * microTone)
                psReal = pitchTools.psToTempered(
                    psBase, octCurrent, self.temperamentObj, trans
                )
    ...
    def _setOrnPos(self, ornPos, rhythmBase, tCurrent, totOrnDur):
        ...
        if totOrnDur > rhythmBase:
            environment.printDebug(
                [
                    "ERROR: pos: %s, baseT: %s, ornT: %s"
                    % (ornPos, rhythmBase, totOrnDur)
                ]
            )
        ...
        if tCurrent - totOrnDur < 0 and ornPos == "anticipate":
            environment.printDebug(
                ["anticipate ornament starts at negative time, mangling to attack"]
            )
            ornPos = "attack"
    ...
    def _getOrnDurStyle(self, presetDict, estOrnDurFraction):
        ...
        if ornStyle == "scale":  # rhythm of note conforms to duration of orn
            ...
            while 1:
                ...
                durCount = durCount + 1  # add to number of durations collected
                if (
                    totOrnDur > (estOrnDurFraction - nextDurInstance)
                    or totOrnDur >= estOrnDurFraction
                ):
        ...
        if totOrnDur > estOrnDurFraction * 1.25:
            ...
            environment.printDebug(
                [
                    "totOrnDur: %s, estOrnDurFraction: %s"
    ...
    def create(
        ...
        presetDict = self._getOrnLibrary(presetName, ornPos, pitchLang, microTone)
        # load parameters
        contourForm = presetDict["contourForm"]
        ornStyle = presetDict["ornStyle"]
        ...
        psBase = pitchTools.psToTempered(
            psBaseRaw, octCurrent, self.temperamentObj, transCurrent
        )
        ...
        psContourReference, refScales = self.psScale(
            pitchLang, contourForm, psBaseRaw, microTone
        )
        ...
        # make base note:
        if durBase > 0.0001:  # if very short, ommit this note (value in seconds)
            abortOrnament = 0
        else:
            abortOrnament = 1
        if not abortOrnament:
            baseNoteEvent = self._makeEventDict(tBaseStart, durBase, amp, psBase, pan)
        else:  # abort ornament, user tempered pitch
            print(lang.WARN, "ornamants aborted")
            baseNoteEvent = self._makeEventDict(tCurrent, rhythmBase, amp, psBase, pan)
        ...
        durPosition = 0
        durBufferSpace = 0.003  # 3 ms gap to avoid clips
        while 1:
            ...
            eventDict = self._makeEventDict(
                tLocalCurrent, durFake, ampInstance, psCurrent, pan
            )  # not complete event
...
class Test(unittest.TestCase):
    ...
    def testDistributions(self):
        ...
        for dur in [0.1, 10, 100, 1000, 100000]:
            for x in range(0, 4):
                post = self.ornObj._addDurNoise(dur, 0.03)

    def testTimeEngine(self):
        self.ornObj = Ornament({}, {})
        ornNotePcent = 0.5
        testCount = 0
        durations = [2.3, 20, 400, 0.01, 0.0005]
            ...
            for name in self.ornObj.ornKeys:
                for ornPos in ["attack", "release", "anticipate"]:
                    for rhythmBase in durations:
                        ...
                        posTimes = self.ornObj._setOrnPos(
                            ornPos, rhythmBase, tCurrent, totOrnDur
                        )
                        durBase, tBaseStart, tBaseEnd, tOrnStart, tOrnEnd = posTimes
                        testCount += 1
