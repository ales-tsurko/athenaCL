...
REFdynStr = { # gives lower boundary value, 8 equal divisions
                ...
                'p' : (.3, .4),
                'pp'    : (.15, .3),
                'ppp' : (.0001, .15), # min sig digits is 4, see below
                'o' : (0, 0), 
                }
...
def dynStrToAcc(sym):
    ...
    if REFdynStr[sym][0] == REFdynStr[sym][1]:
        ...
        return (REFdynStr[sym][0] + REFdynStr[sym][1]) * .5
...
class Pulse:
    ...
    def _scrubDynStr(self, usrStr):
        ...
        for l, sym, val in sortItems:
            if chars.find(sym) >= 0: 
                ...
                if count > 1: continue # not a match
                elif count == 1: match = sym, val # single match
        ...
        if match == None:
            for l, sym, val in sortItems:
                if chars.find(sym) >= 0:  # will start w/ longest first, closest
                    count = chars.count(sym)
                    if count > 1: match = sym, val # accept non-single match
                    else: continue # try again
                if match != None: break           
        if match != None:
    ...
    def _calculate(self, bpm=None, sustain=None):
        ...
        dur = ((bpmToBeatTime(self.bpm) / (float(self.triple[0]))) 
                  * self.triple[1])
    ...
    def _normalizeEventData(self):
        ...
        # if src data is elss, 
            ...
            for i in range(len(self.srcRhythm)):
                self.dstEventData.append(
                    self.srcEventData[i%len(self.srcEventData)])
    ...
    def partition(self):
        ...
        while 1:
            ...
            while 1: # ending this loop means measure is full
                ...
                mSum = mSum + dur
                if mSum == target:
                    if pRemainder != None: tie = 0 # close open tie
                    else: tie = None # no tie
                    if acc == 0: tie = None # case of a rest: no tie
                    ...
                    if pRemainder != None: tie = 2 # continue open tie
                    else: tie = 1 # open a new tie
                    if acc == 0: tie = None # case of a rest: no tie
            ...
            if mCount > 9999999: break # safety 
...
class TimeValue:
    ...
    def _strNum(self, num):
        ...
        if num == 0 or num < .0001:  # min resolution
            return "00"
        elif num > 60:
            return str(num)
        elif num < .01: # show 4 decimals
...
class TestOld:
    ...
    def testRhythm(self):
        testVals = (((2,1),(3,1),(5,1,0)), ('(3,2,1)','(1,1,mf)','(3,1)'),
                      (2,1,1), 'mf,p,f', (1), (1,1), (1,0), (0,1,0), (.2, .4, 1))
