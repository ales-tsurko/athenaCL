...
def _tripleCombine(a, b, direction="+", minMicro=0.000001):
...
def _InvAxisToIndex(axis):
    ...
    index, remainder = divmod(axis, 0.5)
...
def _midiLimit(midiPc, micro=0):
    ...
    if micro >= 0.5:  # round for larger mivrotsones
        midiPc = midiPc + 1
...
def psToNoteName(psReal):
    ...
    # b/n 0-.25; .25-.75; .75-1
    if micro >= 0 and micro < 0.25:
        qStr = ""
        qOffset = 0
    elif micro >= 0.25 and micro < 0.75:
        qStr = SYMqTone
        qOffset = 0
    elif micro >= 0.75 and micro <= 1:
        qStr = ""
        qOffset = 1  # add an extra half step, round up
    else:
        raise ValueError("micro tone value exceeds range")
...
def psToMusicXml(psReal):
    ...
    if micro >= 0 and micro < 0.25:
        ...
        qVal = 0.5  # value here is in half semitones
        qOffset = 0
    elif micro >= 0.75 and micro <= 1:
        qVal = 0
        qOffset = 1  # add an extra half step, round up
...
class Pitch:
    ...
    def _convert(self, dst, src=None, data=None):
        ...
        if src == "midi":
            if dst == "psReal":
                return midiToPs(data)
            elif dst == "psName":
                return midiToNoteName(data)
            elif dst == "fq":
                return midiToFq(data)
            elif dst == "pch":
                return psToPch(midiToPs(data))
            elif dst == "pc":
                return roundMicro(pcTransposer(midiToPs(data), 0))
            elif dst == "midi":
                return roundMicro(data)  # do round here only
        elif src == "psName":
            if dst == "midi":
                return psToMidi(psNameToPs(data))
            elif dst == "psReal":
                return psNameToPs(data)
            elif dst == "fq":
                return psToFq(psNameToPs(data))
            elif dst == "pch":
                return psToPch(psNameToPs(data))
            elif dst == "pc":
                return roundMicro(pcTransposer(psNameToPs(data), 0))
            elif dst == "psName":
                return psToNoteName(self._real)  # use real
        elif src == "psReal":
            if dst == "midi":
                return psToMidi(data)
            elif dst == "psName":
                return psToNoteName(data)
            elif dst == "fq":
                return psToFq(data)
            elif dst == "pch":
                return psToPch(data)
            elif dst == "pc":
                return roundMicro(pcTransposer(data, 0))
            elif dst == "psReal":
                return data
        elif src == "pch":
            if dst == "midi":
                return psToMidi(pchToPs(data))
            elif dst == "psName":
                return psToNoteName(pchToPs(data))
            elif dst == "fq":
                return psToFq(pchToPs(data))
            elif dst == "pch":
                return data
            elif dst == "pc":
                return pcTransposer(roundMicro(pchToPs(data)), 0)
            elif dst == "psReal":
                return pchToPs(data)
        elif src == "fq":
            if dst == "midi":
                return fqToMidi(data)
            elif dst == "psName":
                return fqToNoteName(data)
            elif dst == "fq":
                return data
            elif dst == "pch":
                return fqToPch(data)
            elif dst == "pc":
                return pcTransposer(roundMicro(fqToPs(data)), 0)
            elif dst == "psReal":
                return fqToPs(data)
        return None
