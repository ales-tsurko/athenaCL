...
class StyleSheet:
    ...
    def setBkg(self, bkgImage=None, bkgAttach=None):
        if bkgImage == None: self.bkgImage = ''
        else: self.bkgImage = bkgImage
        if bkgAttach == None: self.bkgAttach = 'fixed' # can be something else
        else: self.bkgAttach = bkgAttach
...
# </STYLE>
    ...
    span {font-size: %spt;
        ...
        color: %s;
        font-weight: normal}
''' % (self.fontDict['mon'], self.fontMono, self.colorDict['mon'], 
         self.fontDict['mon'], self.fontMono, self.colorDict['mon'], 
         self.fontDict['p'],     self.fontBase, self.colorDict['p'], # p
         self.fontDict['p'],     self.fontBase, self.colorDict['p'], 
        ))
...
''' % (self.fontDict['p'], self.fontBase, self.colorDict['p'], 
         self.fontDict['p'], self.fontBase, self.colorDict['p'], # p
         self.fontDict['p'], self.fontBase, self.colorDict['p'], 
         self.fontDict['p'], self.fontBase, self.colorDict['h2'], 

         self.fontDict['h1'], self.fontBase, self.colorDict['h1'], 
         self.fontDict['h2'], self.fontBase, self.colorDict['h2'], 
         self.fontDict['sm'], self.fontBase, self.colorDict['sm'],
         self.fontDict['mon'], self.fontMono, self.colorDict['mon'],
))
...
''' % (self.fontDict['h1'], self.fontBase, self.colorDict['h1'], 
         self.fontDict['h2'], self.fontBase, self.colorDict['h2'], 
         self.fontDict['h3'], self.fontBase, self.colorDict['h3'],
    ))
...
''' % (self.colorDict['hr']))
    ...
    a:Hover {
        ...
        color: %s;}
''' % (self.colorDict['link'], self.colorDict['link'], 
         self.colorDict['post'], self.colorDict['hover']))
...
class ImageNameBank:
    ...
    def __call__(self, parentDir, ext='.gif', group=None, groupMax=None):
        ...
        if self.localMode == 'cgi':
            ...
            new = 0      
        elif (len(self.fileNames[group]) >= self.groupMax[group] and
            self.groupMax[group] != None): # use old
...
"""
    ...
    def linkImage(self, id, postBreaks=0):
        ...
        msg.append("('%s')" % imageName) # type of quotes matters
        msg.append('"><img name="%s" src="%s" alt="%s" border=0></a>' % (
...
"""
    ...
    def linkToPop(self, html=None, label='open', 
            width=200, height=80, postBreaks=1):
        codeC = """
...
""" % (html, label, width, height, label, label)
...
class WebPage:
    def __init__(self, tableW=640, description='', 
                     colorDict=None, fontDict=None,
                     titleRoot='', keywords='', iconURL=''
                      ):
...
""" % (self.titleRoot, pageTitle, self.description, 
         self.keywords, self.iconURL))
        ...
        if css == None: # use basic style sheet
            ...
            cssObj.setFontFace('Trebuchet MS, arial, sans-serif', 
            ...
            if css.endswith('.css'):
                ...
                msg.append("""<link title="new" rel="stylesheet" href="%s" type="text/css"/>""" % cssPath)
    ...
    def tableInit(self, tableWidth, col, color=None, colHeight=1, 
                      colWidth=None, colAlign='left'):
...
<td width='%s' height='%s' align='%s' valign="top">\n""" % 
            (colWidth, colHeight, colAlign))
        else:
            self.tdOpen = """\t
<td width='%s' height='%s' align='%s' valign="top" bgcolor='%s'>\n
"""     % (colWidth, colHeight, colAlign, color)
        self.tdClose = """\t</td>\n"""
    ...
    def imageString(self, target, label='img', postBreaks=1):
        ...
        """
        msg = []
        msg.append("<img src='%s' alt='%s' border='0'>" % (
    ...
    def imageResize(self, target, label='img', w=None, h=None, postBreaks=1):
        ...
        if w == None and h == None:
            ...
            msg.append("<img src='%s' alt='%s' border='0' height='%s'>" % (
                             self.getImagePath(target), label, h))
        elif w != None and h == None:
            msg.append("<img src='%s' alt='%s' border='0' width='%s'>" % (
                                        self.getImagePath(target), label, w))
        elif w != None and h != None:
            msg.append("<img src='%s' alt='%s' border='0' width='%s' height='%s'>" 
    ...
    def linkImage(self, target, label, image, postBreaks=1):
        ...
        target = self.getFilePath(target)
        msg = []
        msg.append('<a href="%s"><img src="%s" alt="%s" border=0></a>' % (
    ...
    def linkImageWindow(self, target, label, image, postBreaks=1):
        ...
        imagePath = self.getImagePath(image)        
        msg = []
        msg.append("<a href='%s' target='_blank'><img src='%s' alt='%s' border=0></a>" % (target, imagePath, label))
        for i in range(0, postBreaks):
            msg.append(self.BR)
        msg.append('\n')
        return ''.join(msg)
    ...
    def linkAmazonId(self, isbn, label, id):
        ...
        e = '&l=as2&o=1&a=%s" width="1" height="1" border="0" alt="" style="border:none !important; margin:0px important!"/>' % isbn
        return a+b+c+d+e
...
''' % dst
    ...
    def framePage(self, sizeList=None, pathList=None, nameList=None):
        ...
        htmlStr="""<html><frameset cols="%s"><frame src="%s" name="%s" frameborder="0" ><frame src="%s" name="%s" frameborder="1"></frameset></html>""" % (
                                                          colStr,
                                                          pathList[0], nameList[0],
                                                          pathList[1], nameList[1])
        return htmlStr
    ...
    def graphicTable(self, source=None, foreColor='#000000', 
                          backColor='#aaaaaa', xMult=1, yMult=1, 
                          rowHeight=1, postBreaks=0):
    ...
    def nTable(self, colList=None, postBreaks=0):
        ...
        if colList == None:
            colList = []
            colList.append({'width':320, 'align':'right', 'color':None, 
                                 'content':'colA' })
            colList.append({'width':15,  'align':'right', 'color':None,
                                 'content':'colB' })
            colList.append({'width':90,  'align':'left',     'color':None,
                                 'content':'colC' })
        totWid = 0
        ...
        for colData in colList:
            ...
            if outline == None: outline = '' # replace w/ empty str
            ...
            if pad != 0:
                # create left pad
                self.tableInit(None, 1, color, 1, int(pad*.5), align) # each column
                msg.append(self.tdOpen)
                msg.append(self.tdClose)

            # create content
            self.tableInit(None, 1, color, 1, 
                                (rawWidth - pad - outlineOffset), align)
            msg.append(self.tdOpen)
            msg.append(content)
            msg.append(self.tdClose)
    ...
    def dataTableAdj(self, label, data, lConfig=[100,'right'], 
                                                    rConfig=[100,'right']):
        # like a data table, but with adjustable width
        colList = []
        colList.append({'width':lConfig[0], 'align':lConfig[1],
                             'color':None, 'content':label })
        colList.append({'width':rConfig[0], 'align':rConfig[1], 
                             'color':None, 'content':data })      
    ...
    def fontSlice(self, sourceStr='this is a test', folder='charTitle', 
                      forceWidth=None, postBreaks=1):
    ...
    def dateStr(self, dateRepr):
        ...
        if drawer.isStr(dateRepr):
            ...
            return '%s, %s %s %s %s' % (dayStr, drawer.intToStr(day, 2), 
                                        monthStr, year, tStrDefault)
    ...
    def formSelect(self, optionPairs=None, postBreaks=0):
        if optionPairs == None:
            optionPairs = (('', '+'),
                                ('http://www.algorithmic.net', 'algorithmic.net'))
        name = 'selector'
        msg = []
...
""" % ('links' + str(random.choice(range(0,100)))))
    ...
    def formTextArea(self, rows=2, label='textMsg', width=160):
        msg = '<textarea name="%s" rows="%s" wrap="physical" style="width: %s; font-size: 10px"></textarea>' % (label, rows, width) # attr: #cols=30
        return msg
...
"""
    ...
    def formSubmit(self, label="Submit", name="submit"):
        return """<input value="%s" name="%s" type="submit" style="width: 60; font-size: 10px">""" % (label, name)

    def formRadioButton(self, group, value, label, style='plain'):
        label = self.pText(label, 'plain', 0)
        msg = '<input TYPE="radio" NAME="%s" VALUE="%s">%s</input>' % (group, 
                 value, label)
        return msg
    ...
    def imageTest(self, name):
        ...
        if (name[-4:] == '.jpg' or name[-4:] == '.gif' or
             name[-4:] == '.png' or name[-5:] == '.jpeg'):
            return 1
        else:
            return 0
...
""" % (ipAddress, port, mountPoint, title)
    ...
    def formatFreeSoundCite(self, audioRef, strProject='This project', 
                                        titleLinkSwitch='name', preBreaks=0):
        ...
        for x in range(preBreaks):
            b.append(self.BR)
        b.append("""%s employs processed (creatively transformed) sounds drawn from %s. These sounds are used under the terms of the %s license, where the following links serve as attribution for the original source sounds.""" % (strProject,             
             self.linkString('http://freesound.iua.upf.edu', 
                          'The Freesound Project', 0),
             self.linkString('http://creativecommons.org/licenses/sampling+/1.0', 
                          'Creative Commons Sampling Plus 1.0', 0)
                    ))
...
""")
    ...
    def writeFramePage(self, htmlPath, sizeList=None, 
                             pathList=None, nameList=None):
        "if no html path (None) prints page to stdout"
...
def getWeather(displayFmt='print'):
    ...
    for locStr in keys:
        ...
        if webpage != None:
            ...
            msg.append('%s temperature: %s (f) %s (c)\n' % (locStr, 
